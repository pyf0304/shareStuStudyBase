...
import {
  QxFuncModule_GetObjLstCache,
  QxFuncModule_GetObjLstByPagerAsync,
  QxFuncModule_SortFunByKey,
...
export const qxFuncModuleEx_Controller = 'QxFuncModuleExApi';
 ...
 **/
export function QxFuncModuleEx_CopyToEx(
  objQxFuncModuleENS: clsQxFuncModuleEN,
): clsQxFuncModuleENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      qxFuncModuleEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function QxFuncModuleEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsQxFuncModuleENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrQxFuncModuleObjLst = await QxFuncModule_GetObjLstByPagerAsync(objPagerPara);
  ...
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrQxFuncModuleSel = arrQxFuncModuleSel.sort(
        QxFuncModuleEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function QxFuncModuleEx_FuncMapByFldName(
  strFldName: string,
  objQxFuncModuleEx: clsQxFuncModuleENEx,
) {
