...
import { vCurrEduCls_GetObjLstByPagerAsync,vCurrEduCls_SortFunByKey,vCurrEduCls_FilterFunByKey } from "@/ts/L3ForWApi/DailyRunning/clsvCurrEduClsWApi";
import { clsvCurrEduClsEN } from "@/ts/L0Entity/DailyRunning/clsvCurrEduClsEN";
import { Format,IsNullOrEmpty } from "@/ts/PubFun/clsString";
import { clsSysPara4WebApi } from "@/ts/PubConfig/clsSysPara4WebApi";
import { stuPagerPara } from "@/ts/PubFun/stuPagerPara";

export const vCurrEduClsExController = "vCurrEduClsExApi";
...
const arrvCurrEduClsObjLst = await vCurrEduCls_GetObjLstByPagerAsync(objPagerPara);
...
let arrvCurrEduClsSel: Array < clsvCurrEduClsENEx > = arrvCurrEduClsExObjLst;
try {
...
arrvCurrEduClsSel = arrvCurrEduClsSel.sort(vCurrEduClsEx_SortFunByKey(strSortFld, strSortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrvCurrEduClsSel = arrvCurrEduClsSel.sort(objPagerPara.sortFun);
}
return arrvCurrEduClsSel;
}
catch (e) {
