...
import { vXzUniZone_GetObjLstByPagerAsync,vXzUniZone_SortFunByKey } from "@/ts/L3ForWApi/SysPara/clsvXzUniZoneWApi";
import { clsvXzUniZoneEN } from "@/ts/L0Entity/SysPara/clsvXzUniZoneEN";
import { Format,IsNullOrEmpty } from "@/ts/PubFun/clsString";
import { clsSysPara4WebApi } from "@/ts/PubConfig/clsSysPara4WebApi";
import { stuPagerPara } from "@/ts/PubFun/stuPagerPara";

export const vXzUniZoneExController = "vXzUniZoneExApi";
...
const arrvXzUniZoneObjLst = await vXzUniZone_GetObjLstByPagerAsync(objPagerPara);
...
let arrvXzUniZoneSel: Array < clsvXzUniZoneENEx > = arrvXzUniZoneExObjLst;
try {
...
arrvXzUniZoneSel = arrvXzUniZoneSel.sort(vXzUniZoneEx_SortFunByKey(strSortFld, strSortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrvXzUniZoneSel = arrvXzUniZoneSel.sort(objPagerPara.sortFun);
}
return arrvXzUniZoneSel;
}
catch (e) {
