...
import {
  ...
  RsStuType_GetObjLstByPagerAsync,
...
export const rsStuTypeExController = 'RsStuTypeExApi';
...
export async function RsStuTypeEx_GetObjExLstByPagerAsync(
  ...
  const arrRsStuTypeObjLst = await RsStuType_GetObjLstByPagerAsync(objPagerPara);
  ...
  let arrRsStuTypeSel: Array<clsRsStuTypeENEx> = arrRsStuTypeExObjLst;
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrRsStuTypeSel = arrRsStuTypeSel.sort(RsStuTypeEx_SortFunByKey(strSortFld, strSortType));
    } else {
