...
export const cc_KnowledgeModulesExController = 'cc_KnowledgeModulesExApi';
...
export async function cc_KnowledgeModulesEx_GetObjExLstByPagerAsync(
  ...
  const arrcc_KnowledgeModulesObjLst = await cc_KnowledgeModules_GetObjLstAsync(
  ...
  if (
    ...
    for (const objInFor of arrcc_KnowledgeModulesExObjLst) {
      await cc_KnowledgeModulesEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
...
export async function cc_KnowledgeModulesEx_GetKeyIdByKnowledgeModuleName(
  strKnowledgeModuleName: string,
  strCourseId: string,
): Promise<string> {
  const strThisFuncName = cc_KnowledgeModulesEx_GetKeyIdByKnowledgeModuleName.name;
  try {
    // const strKeyId = await cc_KnowledgeModulesStore.getKnowledgeModuleIdLstByKnowledgeModuleName(strKnowledgeModuleName)
    const strWhere = `${clscc_KnowledgeModulesEN.con_KnowledgeModuleName} = '${strKnowledgeModuleName}' and 
          ${clscc_KnowledgeModulesEN.con_CourseId} = '${strCourseId}'`;
    const arr = await cc_KnowledgeModules_GetObjLstAsync(strWhere);
    if (arr.length == 0) {
      alert(`条件${strWhere}的表记录数为空！`);
      return '';
    }
    return arr[0].knowledgeModuleId;
  } catch (e) {
    const strMsg = Format(
