...
export async function ge_KnowledgesUserMasterEx_GetObjExLstByPagerAsync(
  ...
  if (
    ...
    clsge_KnowledgesUserMasterEN.AttributeName.indexOf(objSortInfo.SortFld) == -1
  ) {
    for (const objInFor of arrge_KnowledgesUserMasterExObjLst) {
      await ge_KnowledgesUserMasterEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    arrge_KnowledgesUserMasterSel = arrge_KnowledgesUserMasterSel.slice(intStart, intEnd);
...
export async function ge_KnowledgesUserMasterEx_FuncMapMasterTypeName(
  ...
  try {
    ...
    const strMsg = Format(
      '(errid:Watl000508)函数映射表对象数据出错,{0}.(in {1}.{2})',
      e,
      ge_KnowledgesUserMasterEx_ConstructorName,
      strThisFuncName,
    );
