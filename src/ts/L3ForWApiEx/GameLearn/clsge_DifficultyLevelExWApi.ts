...
import { ge_DifficultyLevel_GetObjLstAsync } from '@/ts/L3ForWApi/SystemSet/clsge_DifficultyLevelWApi';
import { ge_DifficultyLevel_SortFunByKey } from '@/ts/L3ForWApi/SystemSet/clsge_DifficultyLevelWApi';
import { Format } from '@/ts/PubFun/clsString';
import { ge_DifficultyLevel_FilterFunByKey } from '@/ts/L3ForWApi/SystemSet/clsge_DifficultyLevelWApi';
import { clsSysPara4WebApi } from '@/ts/PubConfig/clsSysPara4WebApi';
import { clsge_DifficultyLevelEN } from '@/ts/L0Entity/SystemSet/clsge_DifficultyLevelEN';
export const ge_DifficultyLevelEx_Controller = 'ge_DifficultyLevelExApi';
 ...
 * @returns 对象列表
 */
export async function ge_DifficultyLevelEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsge_DifficultyLevelENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrge_DifficultyLevelObjLst = await ge_DifficultyLevel_GetObjLstAsync(
    objPagerPara.whereCond,
  );
  const arrge_DifficultyLevelExObjLst = arrge_DifficultyLevelObjLst.map(
    ge_DifficultyLevelEx_CopyToEx,
  );
  ...
  if (arrge_DifficultyLevelExObjLst.length == 0) return arrge_DifficultyLevelExObjLst;
  let arrge_DifficultyLevel_Sel: Array<clsge_DifficultyLevelENEx> = arrge_DifficultyLevelExObjLst;
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrge_DifficultyLevel_Sel = arrge_DifficultyLevel_Sel.sort(
        ge_DifficultyLevelEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
  ...
  return new Array<clsge_DifficultyLevelENEx>();
}
 ...
 */
export async function ge_DifficultyLevelEx_FilterFunByKey(strKey: string, value: any) {
  const strThisFuncName = 'FilterFunByKey';
  let strMsg = '';
  switch (strKey) {
    default:
      return ge_DifficultyLevel_FilterFunByKey(strKey, value);
  }
}
