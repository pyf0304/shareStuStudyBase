...
import { ge_OperationType_GetObjLstAsync } from '@/ts/L3ForWApi/GameLearn/clsge_OperationTypeWApi';
import { ge_OperationType_SortFunByKey } from '@/ts/L3ForWApi/GameLearn/clsge_OperationTypeWApi';
import { Format } from '@/ts/PubFun/clsString';
import { ge_OperationType_FilterFunByKey } from '@/ts/L3ForWApi/GameLearn/clsge_OperationTypeWApi';
import { clsSysPara4WebApi } from '@/ts/PubConfig/clsSysPara4WebApi';
export const ge_OperationTypeEx_Controller = 'ge_OperationTypeExApi';
...
export async function ge_OperationTypeEx_GetObjExLstByPagerAsync(
  ...
  const arrge_OperationTypeObjLst = await ge_OperationType_GetObjLstAsync(objPagerPara.whereCond);
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrge_OperationTypeExObjLst) {
      const conFuncMap = await ge_OperationTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  if (arrge_OperationTypeExObjLst.length == 0) return arrge_OperationTypeExObjLst;
  let arrge_OperationType_Sel: Array<clsge_OperationTypeENEx> = arrge_OperationTypeExObjLst;
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrge_OperationType_Sel = arrge_OperationType_Sel.sort(
        ge_OperationTypeEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
      //如果排序字段名[OrderBy]为空，就调用排序函数
      arrge_OperationType_Sel = arrge_OperationType_Sel.sort(objPagerPara.sortFun);
    }
    arrge_OperationType_Sel = arrge_OperationType_Sel.slice(intStart, intEnd);
    return arrge_OperationType_Sel;
  } catch (e) {
    const strMsg = Format(
 ...
 */
export async function ge_OperationTypeEx_FilterFunByKey(strKey: string, value: any) {
  const strThisFuncName = 'FilterFunByKey';
  let strMsg = '';
  switch (strKey) {
    default:
      return ge_OperationType_FilterFunByKey(strKey, value);
  }
}
