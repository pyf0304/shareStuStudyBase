...
export async function vUsersSimEx_GetObjExLstByPagerAsync(
  ...
  const arrvUsersSimObjLst = await vUsersSim_GetObjLstByPagerAsync(objPagerPara);
  ...
  let arrvUsersSimSel: Array<clsvUsersSimENEx> = arrvUsersSimExObjLst;
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvUsersSimSel = arrvUsersSimSel.sort(vUsersSimEx_SortFunByKey(strSortFld, strSortType));
    } else {
