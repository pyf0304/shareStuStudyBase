...
import { vStudentInfo_GetObjLstByPagerAsync,vStudentInfo_SortFunByKey } from "@/ts/L3ForWApi/UserManage/clsvStudentInfoWApi";
import { clsvStudentInfoEN } from "@/ts/L0Entity/UserManage/clsvStudentInfoEN";
import { Format,IsNullOrEmpty } from "@/ts/PubFun/clsString";
import { clsSysPara4WebApi } from "@/ts/PubConfig/clsSysPara4WebApi";
import { stuPagerPara } from "@/ts/PubFun/stuPagerPara";

export const vStudentInfoExController = "vStudentInfoExApi";
 ...
 **/
export  function vStudentInfoEx_CopyToEx(objvStudentInfoENS:clsvStudentInfoEN ): clsvStudentInfoENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, vStudentInfoEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function vStudentInfoEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsvStudentInfoENEx>> {
const strThisFuncName = "GetObjExLstByPagerAsync";
const arrvStudentInfoObjLst = await vStudentInfo_GetObjLstByPagerAsync(objPagerPara);
...
let arrvStudentInfoSel: Array < clsvStudentInfoENEx > = arrvStudentInfoExObjLst;
try {
...
arrvStudentInfoSel = arrvStudentInfoSel.sort(vStudentInfoEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, vStudentInfoEx_ConstructorName, strThisFuncName);
console.error(strMsg);
...
*/
export  function vStudentInfoEx_FuncMapByFldName(strFldName: string, objvStudentInfoEx: clsvStudentInfoENEx)
{
