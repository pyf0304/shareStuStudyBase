...
import { SourceType_GetObjLstCache } from '@/ts/L3ForWApi/InteractManage/clsSourceTypeWApi';
...
import { SourceType_GetObjLstAsync } from '@/ts/L3ForWApi/InteractManage/clsSourceTypeWApi';
import { SourceType_SortFunByKey } from '@/ts/L3ForWApi/InteractManage/clsSourceTypeWApi';
import { Format } from '@/ts/PubFun/clsString';
import { SourceType_FilterFunByKey } from '@/ts/L3ForWApi/InteractManage/clsSourceTypeWApi';
import { clsSysPara4WebApi } from '@/ts/PubConfig/clsSysPara4WebApi';
export const sourceTypeEx_Controller = 'SourceTypeExApi';
...
export async function SourceTypeEx_GetObjExLstByPagerAsync(
  ...
  const arrSourceTypeObjLst = await SourceType_GetObjLstAsync(objPagerPara.whereCond);
  const arrSourceTypeExObjLst = arrSourceTypeObjLst.map(SourceTypeEx_CopyToEx);
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrSourceTypeExObjLst) {
      const conFuncMap = await SourceTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  if (arrSourceTypeExObjLst.length == 0) return arrSourceTypeExObjLst;
  let arrSourceType_Sel: Array<clsSourceTypeENEx> = arrSourceTypeExObjLst;
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrSourceType_Sel = arrSourceType_Sel.sort(
        SourceTypeEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
      //如果排序字段名[OrderBy]为空，就调用排序函数
      arrSourceType_Sel = arrSourceType_Sel.sort(objPagerPara.sortFun);
    }
    arrSourceType_Sel = arrSourceType_Sel.slice(intStart, intEnd);
    return arrSourceType_Sel;
  } catch (e) {
    const strMsg = Format(
 ...
 */
export async function SourceTypeEx_FilterFunByKey(strKey: string, value: any) {
  const strThisFuncName = 'FilterFunByKey';
  let strMsg = '';
  switch (strKey) {
    default:
      return SourceType_FilterFunByKey(strKey, value);
  }
}
