import { cacheUseStateCache } from '@/viewsShare/SystemSet/CacheUseStateVueShare';
import { isFuncMapCache } from '@/viewsShare/SystemSet/CacheUseStateVueShare';
...
export async function CacheUseStateEx_GetObjExLstByPagerCache(
  ...
  const arrCacheUseStateExObjLst = arrCacheUseStateObjLst.map((obj) => {
    ...
    if (cacheUseStateCache[cacheKey]) {
      ...
      const newObj = CacheUseStateEx_CopyToEx(obj);
  ...
  for (const newObj of arrNewObj) {
    for (const strFldName of Object.keys(isFuncMapCache)) {
      await CacheUseStateEx_FuncMapByFldName(strFldName, newObj);
    }
  }
  ...
  if (
    ...
    for (const newObj of arrCacheUseStateExObjLst) {
      await CacheUseStateEx_FuncMapByFldName(objSortInfo.SortFld, newObj);
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      arrCacheUseStateSel = arrCacheUseStateSel.sort(
        CacheUseStateEx_SortFunByKey(objSortInfo.SortFld, objSortInfo.SortType),
      );
    } else {
