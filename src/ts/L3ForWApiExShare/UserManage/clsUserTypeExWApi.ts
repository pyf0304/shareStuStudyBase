...
import {
  ...
  UserType_GetObjLstAsync,
  UserType_SortFunByKey,
} from '@/ts/L3ForWApi/UserManage/clsUserTypeWApi';
import { stuPagerPara } from '@/ts/PubFun/stuPagerPara';
import { IsNullOrEmpty, Format } from '@/ts/PubFun/clsString';
import { clsUserTypeEN } from '@/ts/L0Entity/UserManage/clsUserTypeEN';
import { clsUserTypeENEx } from '@/ts/L0Entity/UserManage/clsUserTypeENEx';
 ...
 **/
export function UserTypeEx_GetWebApiUrl(strController: string, strAction: string): string {
...
export async function UserTypeEx_GetObjExLstByPagerAsync(
  ...
  const arrUserTypeObjLst = await UserType_GetObjLstAsync(objPagerPara.whereCond);
 ...
 * @returns 返回两个对象比较的结果
 */
export function UserTypeEx_SortFunByKey(strKey: string, AscOrDesc: string) {
