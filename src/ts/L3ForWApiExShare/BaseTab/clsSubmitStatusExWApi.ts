...
import {
  SubmitStatus_GetObjLstCache,
  SubmitStatus_GetObjLstByPagerAsync,
  SubmitStatus_SortFunByKey,
 ...
 **/
export function SubmitStatusEx_CopyToEx(
  objSubmitStatusENS: clsSubmitStatusEN,
): clsSubmitStatusENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      submitStatusEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function SubmitStatusEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsSubmitStatusENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrSubmitStatusObjLst = await SubmitStatus_GetObjLstByPagerAsync(objPagerPara);
  ...
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrSubmitStatusSel = arrSubmitStatusSel.sort(
        SubmitStatusEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function SubmitStatusEx_FuncMapByFldName(
  strFldName: string,
  objSubmitStatusEx: clsSubmitStatusENEx,
) {
