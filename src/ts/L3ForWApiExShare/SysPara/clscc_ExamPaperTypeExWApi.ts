...
import {
  cc_ExamPaperType_GetObjLstCache,
  cc_ExamPaperType_GetObjLstByPagerAsync,
  cc_ExamPaperType_SortFunByKey,
} from '@/ts/L3ForWApi/SysPara/clscc_ExamPaperTypeWApi';
...
export const cc_ExamPaperTypeExController = 'cc_ExamPaperTypeExApi';
...
export async function cc_ExamPaperTypeEx_GetObjExLstByPagerAsync(
  ...
  const arrcc_ExamPaperTypeObjLst = await cc_ExamPaperType_GetObjLstByPagerAsync(objPagerPara);
  ...
  let arrcc_ExamPaperTypeSel: Array<clscc_ExamPaperTypeENEx> = arrcc_ExamPaperTypeExObjLst;
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrcc_ExamPaperTypeSel = arrcc_ExamPaperTypeSel.sort(
...
export function cc_ExamPaperTypeEx_FuncMapByFldName(
  ...
  console.log(objcc_ExamPaperTypeEx);
  let strMsg = '';
  //如果是本表中字段,不需要映射
