...
import { XzSchool_GetObjLstCache } from '@/ts/L3ForWApi/SystemSet/clsXzSchoolWApi';
...
import { XzSchool_GetObjLstAsync } from '@/ts/L3ForWApi/SystemSet/clsXzSchoolWApi';
import { XzSchool_SortFunByKey } from '@/ts/L3ForWApi/SystemSet/clsXzSchoolWApi';
import { Format } from '@/ts/PubFun/clsString';
import { XzSchool_FilterFunByKey } from '@/ts/L3ForWApi/SystemSet/clsXzSchoolWApi';
...
export async function XzSchoolEx_GetObjExLstByPagerAsync(
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrXzSchoolExObjLst) {
      const conFuncMap = await XzSchoolEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  if (arrXzSchoolExObjLst.length == 0) return arrXzSchoolExObjLst;
  let arrXzSchool_Sel: Array<clsXzSchoolENEx> = arrXzSchoolExObjLst;
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrXzSchool_Sel = arrXzSchool_Sel.sort(XzSchoolEx_SortFunByKey(strSortFld, strSortType));
    } else {
 ...
 */
export async function XzSchoolEx_FilterFunByKey(strKey: string, value: any) {
  const strThisFuncName = 'FilterFunByKey';
  let strMsg = '';
  switch (strKey) {
    default:
      return XzSchool_FilterFunByKey(strKey, value);
  }
}
