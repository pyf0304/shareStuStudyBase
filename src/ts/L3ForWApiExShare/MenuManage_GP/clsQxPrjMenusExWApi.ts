...
export const qxPrjMenusEx_Controller = 'QxPrjMenusExApi';
...
export async function QxPrjMenusEx_FuncMapPageDispModeName(objQxPrjMenus: clsQxPrjMenusENEx) {
  ...
  try {
    ...
    const strMsg = Format(
      '(errid:Watl000598)函数映射表对象数据出错,{0}.(in {1}.{2})',
      e,
      qxPrjMenusEx_ConstructorName,
      strThisFuncName,
    );
  ...
  if (AscOrDesc == 'Asc' || AscOrDesc == '') {
    switch (strKey) {
      ...
      case clsQxPrjMenusENEx.con_MenuNameEx:
        return (a: clsQxPrjMenusENEx, b: clsQxPrjMenusENEx) => {
          return a.menuNameEx.localeCompare(b.menuNameEx);
        };
      default:
    ...
    switch (strKey) {
      ...
      case clsQxPrjMenusENEx.con_MenuNameEx:
        return (a: clsQxPrjMenusENEx, b: clsQxPrjMenusENEx) => {
          return b.menuNameEx.localeCompare(a.menuNameEx);
        };
      default:
        return QxPrjMenus_SortFunByKey(strKey, AscOrDesc);
    }
...
export async function QxPrjMenusEx_FuncMapKeyPageDispModeName(
  ...
  try {
    ...
    const strMsg = Format(
      '(errid:Watl000603)函数映射表对象数据出错,{0}.(in {1}.{2})',
      e,
      qxPrjMenusEx_ConstructorName,
      strThisFuncName,
    );
