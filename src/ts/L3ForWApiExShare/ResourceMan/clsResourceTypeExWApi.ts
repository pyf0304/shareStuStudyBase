...
import {
  ResourceType_GetObjLstCache,
  ResourceType_GetObjLstByPagerAsync,
  ResourceType_SortFunByKey,
...
export const resourceTypeExController = 'ResourceTypeExApi';
...
export async function ResourceTypeEx_GetObjExLstByPagerAsync(
  ...
  const arrResourceTypeObjLst = await ResourceType_GetObjLstByPagerAsync(objPagerPara);
  ...
  let arrResourceTypeSel: Array<clsResourceTypeENEx> = arrResourceTypeExObjLst;
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrResourceTypeSel = arrResourceTypeSel.sort(
        ResourceTypeEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
      //如果排序字段名[OrderBy]为空,就调用排序函数
      arrResourceTypeSel = arrResourceTypeSel.sort(objPagerPara.sortFun);
    }
    return arrResourceTypeSel;
  } catch (e) {
    const strMsg = Format(
