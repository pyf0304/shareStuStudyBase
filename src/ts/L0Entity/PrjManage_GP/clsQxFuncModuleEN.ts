...
export class  clsQxFuncModuleEN extends clsGeneralTab
{
...
public static _CurrTabName= "QxFuncModule"; //当前表名,与该类相关的表名
public static _KeyFldName= "FuncModuleId"; //当前表中的关键字名称,与该类相关的表中关键字名
public static mintAttributeCount = 10;
public static AttributeName = ["funcModuleId", "funcModuleName", "funcModuleNameSim", "funcModuleEnName", "orderNum", "inUse", "qxPrjId", "updDate", "updUser", "memo"];
//以下是属性变量
...
default:
strMsg = `字段名:[${strFldName}]在表对象:[QxFuncModule]中不存在!`;
...
break;
default:
strMsg = `字段名:[${strFldName}]在表对象:[QxFuncModule]中不存在!(in ${this.constructor.name}.${strThisFuncName})`;
console.error(strMsg);
break;
}
}
...
//return propName in new clsQxFuncModuleEN();
const instance = new clsQxFuncModuleEN();
