...
export class  clsge_ControlAttributeEN extends clsGeneralTab
{
...
private mstrControlAttributeName = "";    //属性名称
 ...
 * (AutoGCLib.EntityLayer4TypeScript:Gen_EN_ClsProperty)
*/
 public SetControlAttributeName (value: string)
{
if (value  != undefined)
{
 this.controlAttributeName = value;
...
break;
default:
strMsg = `字段名:[${strFldName}]在表对象:[ge_ControlAttribute]中不存在!(in ${this.constructor.name}.${strThisFuncName})`;
console.error(strMsg);
break;
}
}
...
//return propName in new clsge_ControlAttributeEN();
const instance = new clsge_ControlAttributeEN();
