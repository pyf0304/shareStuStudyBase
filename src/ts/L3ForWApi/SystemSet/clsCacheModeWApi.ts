 ...
 **/
export  async function CacheMode_AddNewObjSave(objCacheModeEN: clsCacheModeEN): Promise<AddRecordResult>{
 ...
 **/
export  async function CacheMode_CheckUniCond4Update(objCacheModeEN: clsCacheModeEN): Promise<boolean>{
 ...
 * @returns 获取修改是否成功？
 **/
export  async function CacheMode_EditRecordExAsync(objCacheModeEN: clsCacheModeEN): Promise<boolean>  
{
...
let arrCacheModeSel = arrCacheModeObjLstCache;
if (objCacheModeCond.GetConditions().length == 0) return arrCacheModeSel.length>0?true:false;
try {
for (const objCondition of objCacheModeCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
