 ...
 **/
export  async function CacheUseState_GetFldValueAsync(strFldName: string, strWhereCond: string): Promise<Array<string>>  
{
 ...
 * @returns 对象列表子集
*/
export  async function CacheUseState_GetSubObjLstCache(objCacheUseStateCond: ConditionCollection ,strUserId: string) {
 ...
 * @returns 对象列表
*/
export  async function CacheUseState_GetObjExLstByPagerCache(objPagerPara: stuPagerPara , strUserId:string):Promise<Array<clsCacheUseStateENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
...
const intEnd = intStart + objPagerPara.pageSize;
if (objPagerPara.orderBy != null && objPagerPara.orderBy.length>0) {
arrCacheUseStateSel = arrCacheUseStateSel.sort(CacheUseState_SortFunByExKey(objSortInfo.SortFld, objSortInfo.SortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrCacheUseStateSel = arrCacheUseStateSel.sort(objPagerPara.sortFun);
 ...
 **/
export  function CacheUseState_CopyToEx(objCacheUseStateENS:clsCacheUseStateEN ): clsCacheUseStateENEx
{
...
{
const strMsg = Format("(errid:Watl001294)Copy表对象数据出错,{0}.(in {1}.{2})", e, cacheUseState_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
 return objCacheUseStateENT;
}
...
*/
export  function CacheUseState_FuncMapByFldName(strFldName: string, objCacheUseStateEx: clsCacheUseStateENEx)
{
...
if (IsNullOrEmpty(objCacheUseState.cacheModeName) == true){
 ...
 const CacheModeCacheModeName = await CacheMode_func(clsCacheModeEN.con_CacheModeId, clsCacheModeEN.con_CacheModeName, CacheModeCacheModeId );
 objCacheUseState.cacheModeName = CacheModeCacheModeName;
}
}
catch (e)
{
const strMsg = Format("(errid:Watl001313)函数映射表对象数据出错,{0}.(in {1}.{2})", e, cacheUseState_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
if (IsNullOrEmpty(objCacheUseState.cacheModeENName) == true){
 const CacheModeCacheModeId = objCacheUseState.cacheModeId;
 const CacheModeCacheModeENName = await CacheMode_func(clsCacheModeEN.con_CacheModeId, clsCacheModeEN.con_CacheModeENName, CacheModeCacheModeId );
 objCacheUseState.cacheModeENName = CacheModeCacheModeENName;
}
}
catch (e)
{
const strMsg = Format("(errid:Watl001314)函数映射表对象数据出错,{0}.(in {1}.{2})", e, cacheUseState_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
 ...
 **/
export  async function CacheUseState_AddNewObjSave(objCacheUseStateEN: clsCacheUseStateEN): Promise<AddRecordResult>{
 ...
 **/
export  async function CacheUseState_CheckUniCond4Add(objCacheUseStateEN: clsCacheUseStateEN): Promise<boolean>{
 ...
 **/
export  async function CacheUseState_CheckUniCond4Update(objCacheUseStateEN: clsCacheUseStateEN): Promise<boolean>{
 ...
 **/
export  async function CacheUseState_UpdateObjSave(objCacheUseStateEN: clsCacheUseStateEN): Promise<boolean>{
 ...
 * @returns 获取修改是否成功？
 **/
export  async function CacheUseState_EditRecordExAsync(objCacheUseStateEN: clsCacheUseStateEN): Promise<boolean>  
{
 ...
 * @returns 对象列表子集
*/
export  async function CacheUseState_IsExistRecordCache(objCacheUseStateCond: ConditionCollection,strUserId: string) {
const strThisFuncName = "IsExistRecordCache";
const arrCacheUseStateObjLstCache = await CacheUseState_GetObjLstCache(strUserId);
if (arrCacheUseStateObjLstCache == null) return false;
let arrCacheUseStateSel = arrCacheUseStateObjLstCache;
if (objCacheUseStateCond.GetConditions().length == 0) return arrCacheUseStateSel.length>0?true:false;
try {
for (const objCondition of objCacheUseStateCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
 ...
 * @returns 对象列表记录数
*/
export  async function CacheUseState_GetRecCountByCondCache(objCacheUseStateCond: ConditionCollection ,strUserId: string) {
...
if (IsNullOrEmpty(strUserId) == true)
{
  const strMsg = Format("参数:[strUserId]不能为空!(In clsCacheUseStateWApi.CacheUseState_ReFreshThisCache)");
console.error(strMsg);
 throw (strMsg);
}
