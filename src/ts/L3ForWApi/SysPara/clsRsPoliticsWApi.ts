 ...
 * @returns 对象列表子集
*/
export  async function RsPolitics_GetSubObjLstCache(objRsPoliticsCond: ConditionCollection ) {
const strThisFuncName = "GetSubObjLstCache";
const arrRsPoliticsObjLstCache = await RsPolitics_GetObjLstCache();
let arrRsPoliticsSel = arrRsPoliticsObjLstCache;
if (objRsPoliticsCond.GetConditions().length == 0) return arrRsPoliticsSel;
try {
//console.log(sstrKeys);
for (const objCondition of objRsPoliticsCond.GetConditions()) {
 ...
 * @returns 对象列表子集
*/
export  async function RsPolitics_IsExistRecordCache(objRsPoliticsCond: ConditionCollection) {
const strThisFuncName = "IsExistRecordCache";
const arrRsPoliticsObjLstCache = await RsPolitics_GetObjLstCache();
if (arrRsPoliticsObjLstCache == null) return false;
let arrRsPoliticsSel = arrRsPoliticsObjLstCache;
if (objRsPoliticsCond.GetConditions().length == 0) return arrRsPoliticsSel.length>0?true:false;
try {
for (const objCondition of objRsPoliticsCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
 ...
 * @returns 对象列表记录数
*/
export  async function RsPolitics_GetRecCountByCondCache(objRsPoliticsCond: ConditionCollection ) {
const strThisFuncName = "GetRecCountByCondCache";
const arrRsPoliticsObjLstCache = await RsPolitics_GetObjLstCache();
if (arrRsPoliticsObjLstCache == null) return 0;
let arrRsPoliticsSel = arrRsPoliticsObjLstCache;
if (objRsPoliticsCond.GetConditions().length == 0) return arrRsPoliticsSel.length;
try {
for (const objCondition of objRsPoliticsCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
