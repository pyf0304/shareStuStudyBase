 ...
 **/
export  function KnowledgeLearnTotal_CopyToEx(objKnowledgeLearnTotalENS:clsKnowledgeLearnTotalEN ): clsKnowledgeLearnTotalENEx
{
const strThisFuncName  = KnowledgeLearnTotal_CopyToEx.name;
...
{
const strMsg = Format("(errid:Watl001294)Copy表对象数据出错,{0}.(in {1}.{2})", e, knowledgeLearnTotal_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
 return objKnowledgeLearnTotalENT;
}
...
*/
export  function KnowledgeLearnTotal_FuncMapByFldName(strFldName: string, objKnowledgeLearnTotalEx: clsKnowledgeLearnTotalENEx)
{
const strThisFuncName = KnowledgeLearnTotal_FuncMapByFldName.name;
...
return KnowledgeLearnTotal_FuncMapKnowledgeName(objKnowledgeLearnTotalEx);
        default:
 ...
 * @returns 返回两个对象比较的结果
*/
export  function KnowledgeLearnTotal_SortFunByExKey(strKey:string, AscOrDesc: string)
{
 ...
 **/
export  async function KnowledgeLearnTotal_FuncMapKnowledgeName(objKnowledgeLearnTotal:clsKnowledgeLearnTotalENEx )
{
const strThisFuncName = KnowledgeLearnTotal_FuncMapKnowledgeName.name;
try
{
...
if (IsNullOrEmpty(objKnowledgeLearnTotal.courseId) == true)
{
const strMsg = `函数映射[KnowledgeName]数据出错,courseId不能为空!(in ${ knowledgeLearnTotal_ConstructorName }.${strThisFuncName})`
console.error(strMsg);
alert(strMsg);
return;
}
...
{
const strMsg = Format("(errid:Watl001447)函数映射表对象数据出错,{0}.(in {1}.{2})", e, knowledgeLearnTotal_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
