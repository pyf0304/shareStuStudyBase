...
const strUrl = GetWebApiUrl_GP(qxUserIdentity_Controller, strAction);
 ...
 **/
export  async function QxUserIdentity_GetFldValueAsync(strFldName: string, strWhereCond: string): Promise<Array<string>>  
{
...
let strUrl = GetWebApiUrl_GP(qxUserIdentity_Controller, strAction);
 ...
 **/
export  async function QxUserIdentity_UpdateObjSave(objQxUserIdentityEN: clsQxUserIdentityEN): Promise<boolean>{
 ...
 * @returns 获取修改是否成功？
 **/
export  async function QxUserIdentity_EditRecordExAsync(objQxUserIdentityEN: clsQxUserIdentityEN): Promise<boolean>  
{
...
let arrQxUserIdentitySel = arrQxUserIdentityObjLstCache;
if (objQxUserIdentityCond.GetConditions().length == 0) return arrQxUserIdentitySel.length>0?true:false;
try {
for (const objCondition of objQxUserIdentityCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
 ...
 * @returns 对象列表记录数
*/
export  async function QxUserIdentity_GetRecCountByCondCache(objQxUserIdentityCond: ConditionCollection ) {
...
{
 throw new Error(`(errid:Watl000413)字段[身份编号(identityId)]的长度不能超过2(In 用户权限身份(QxUserIdentity))!值:${pobjQxUserIdentityEN.identityId}(clsQxUserIdentityBL:CheckPropertyNew)`);
}
...
{
 throw new Error(`(errid:Watl000414)字段[身份编号(identityId)]的值:[${pobjQxUserIdentityEN.identityId}], 非法,应该为字符型(In 用户权限身份(QxUserIdentity))!(clsQxUserIdentityBL:CheckPropertyNew0)`);
}
...
if (IsNullOrEmpty(pobjQxUserIdentityEN.identityId) == false && GetStrLen(pobjQxUserIdentityEN.identityId) > 2)
{
 throw new Error(`(errid:Watl000416)字段[身份编号(identityId)]的长度不能超过2(In 用户权限身份(QxUserIdentity))!值:${pobjQxUserIdentityEN.identityId}(clsQxUserIdentityBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjQxUserIdentityEN.identityDesc) == false && GetStrLen(pobjQxUserIdentityEN.identityDesc) > 20)
{
...
if (IsNullOrEmpty(pobjQxUserIdentityEN.identityId) == false && undefined !== pobjQxUserIdentityEN.identityId && tzDataType.isString(pobjQxUserIdentityEN.identityId) === false)
{
 throw new Error(`(errid:Watl000417)字段[身份编号(identityId)]的值:[${pobjQxUserIdentityEN.identityId}], 非法,应该为字符型(In 用户权限身份(QxUserIdentity))!(clsQxUserIdentityBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjQxUserIdentityEN.identityDesc) == false && undefined !== pobjQxUserIdentityEN.identityDesc && tzDataType.isString(pobjQxUserIdentityEN.identityDesc) === false)
{
...
{
 throw new Error(`(errid:Watl000064)字段[身份编号]不能为空(In 用户权限身份)!(clsQxUserIdentityBL:CheckProperty4Update)`);
}
//检查外键, 作为外键应该和主键的字段长度是一样的, 若不一样,即非法!
