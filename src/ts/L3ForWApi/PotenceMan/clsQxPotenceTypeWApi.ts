...
if (IsNullOrEmpty(clsQxPotenceTypeEN.WhereFormat) == false)
{
strWhereCond = clsQxPotenceTypeEN.WhereFormat;
}
 ...
 * @returns 对象列表子集
*/
export  async function QxPotenceType_GetSubObjLstCache(objQxPotenceTypeCond: ConditionCollection ) {
const strThisFuncName = "GetSubObjLstCache";
const arrQxPotenceTypeObjLstCache = await QxPotenceType_GetObjLstCache();
let arrQxPotenceTypeSel = arrQxPotenceTypeObjLstCache;
if (objQxPotenceTypeCond.GetConditions().length == 0) return arrQxPotenceTypeSel;
try {
 ...
 **/
export  async function QxPotenceType_AddNewObjSave(objQxPotenceTypeEN: clsQxPotenceTypeEN): Promise<AddRecordResult>{
...
{
const strMsg = `检查数据不成功,${e}.(in ${ qxPotenceType_ConstructorName }.${strThisFuncName})`;
console.error(strMsg);
alert(strMsg);
...
{
//检查唯一性条件
const bolIsExistCond = await QxPotenceType_CheckUniCond4Add(objQxPotenceTypeEN);
...
{
const strInfo = `添加[权限类型(QxPotenceType)]记录不成功!`;
//显示信息框
throw(strInfo);
}
...
{
const strMsg = `添加记录不成功,${e}.(in ${ qxPotenceType_ConstructorName }.${ strThisFuncName })`;
console.error(strMsg);
throw(strMsg);
}
}
 ...
 **/
export  async function QxPotenceType_CheckUniCond4Add(objQxPotenceTypeEN: clsQxPotenceTypeEN): Promise<boolean>{
 ...
 **/
export  async function QxPotenceType_CheckUniCond4Update(objQxPotenceTypeEN: clsQxPotenceTypeEN): Promise<boolean>{
 ...
 **/
export  async function QxPotenceType_UpdateObjSave(objQxPotenceTypeEN: clsQxPotenceTypeEN): Promise<boolean>{
...
{
//检查唯一性条件
const bolIsExistCond = await QxPotenceType_CheckUniCond4Update(objQxPotenceTypeEN);
if (bolIsExistCond == false)
{
return false;
}
...
{
const strMsg = `修改记录不成功,${e}.(in ${ qxPotenceType_ConstructorName }.${ strThisFuncName })`;
console.error(strMsg);
throw(strMsg);
}
}
 ...
 * @returns 获取修改是否成功？
 **/
export  async function QxPotenceType_EditRecordExAsync(objQxPotenceTypeEN: clsQxPotenceTypeEN): Promise<boolean>  
{
 ...
 * @returns 对象列表子集
*/
export  async function QxPotenceType_IsExistRecordCache(objQxPotenceTypeCond: ConditionCollection) {
const strThisFuncName = "IsExistRecordCache";
const arrQxPotenceTypeObjLstCache = await QxPotenceType_GetObjLstCache();
if (arrQxPotenceTypeObjLstCache == null) return false;
let arrQxPotenceTypeSel = arrQxPotenceTypeObjLstCache;
if (objQxPotenceTypeCond.GetConditions().length == 0) return arrQxPotenceTypeSel.length>0?true:false;
try {
for (const objCondition of objQxPotenceTypeCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
 ...
 * @returns 对象列表记录数
*/
export  async function QxPotenceType_GetRecCountByCondCache(objQxPotenceTypeCond: ConditionCollection ) {
const strThisFuncName = "GetRecCountByCondCache";
const arrQxPotenceTypeObjLstCache = await QxPotenceType_GetObjLstCache();
if (arrQxPotenceTypeObjLstCache == null) return 0;
let arrQxPotenceTypeSel = arrQxPotenceTypeObjLstCache;
if (objQxPotenceTypeCond.GetConditions().length == 0) return arrQxPotenceTypeSel.length;
try {
for (const objCondition of objQxPotenceTypeCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
...
clsQxPotenceTypeEN._RefreshTimeLst.push(clsDateTime.getTodayDateTimeStr(0));
}
...
**/
export  function QxPotenceType_GetLastRefreshTime():string
{
if (clsQxPotenceTypeEN._RefreshTimeLst.length == 0) return "";
return clsQxPotenceTypeEN._RefreshTimeLst[clsQxPotenceTypeEN._RefreshTimeLst.length - 1];
}
...
*/
export  async function QxPotenceType_GetArrQxPotenceType()
{

//为数据源于表的下拉框设置内容
//console.log("开始：BindDdl_PotenceTypeIdInDivCache");
const arrQxPotenceType = new Array<clsQxPotenceTypeEN>();
const arrObjLstSel = await QxPotenceType_GetObjLstCache();
if (arrObjLstSel == null) return null;
const obj0 = new clsQxPotenceTypeEN();
obj0.potenceTypeId = '0';
obj0.potenceTypeName = '选权限类型...';
arrQxPotenceType.push(obj0);
arrObjLstSel.forEach(x => arrQxPotenceType.push(x));
return arrQxPotenceType;
}
