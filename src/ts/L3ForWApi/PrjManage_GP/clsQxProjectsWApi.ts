 ...
 * @returns 对象列表子集
*/
export  async function QxProjects_GetSubObjLstCache(objQxProjectsCond: ConditionCollection ) {
const strThisFuncName = "GetSubObjLstCache";
const arrQxProjectsObjLstCache = await QxProjects_GetObjLstCache();
let arrQxProjectsSel = arrQxProjectsObjLstCache;
if (objQxProjectsCond.GetConditions().length == 0) return arrQxProjectsSel;
try {
//console.log(sstrKeys);
for (const objCondition of objQxProjectsCond.GetConditions()) {
...
let arrQxProjectsSel = arrQxProjectsObjLstCache;
const objQxProjectsCond = objPagerPara.conditionCollection;
if (objQxProjectsCond == null)
{
...
let arrQxProjectsSel = arrQxProjectsObjLstCache;
if (objQxProjectsCond.GetConditions().length == 0) return arrQxProjectsSel.length>0?true:false;
try {
for (const objCondition of objQxProjectsCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
 ...
 * @returns 对象列表记录数
*/
export  async function QxProjects_GetRecCountByCondCache(objQxProjectsCond: ConditionCollection ) {
const strThisFuncName = "GetRecCountByCondCache";
const arrQxProjectsObjLstCache = await QxProjects_GetObjLstCache();
if (arrQxProjectsObjLstCache == null) return 0;
let arrQxProjectsSel = arrQxProjectsObjLstCache;
if (objQxProjectsCond.GetConditions().length == 0) return arrQxProjectsSel.length;
try {
for (const objCondition of objQxProjectsCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
