...
{
strWhereCond += Format(" And tz_ContentAttachment.{0} = '{1}'", clstz_ContentAttachmentEN.con_IdResourceType, idResourceType_q.value);
}

}
catch(objException)
{
const strMsg:string = Format("在组合查询条件(Combinetz_ContentAttachmentCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
};
 ...
 * @returns 条件串(strWhereCond)
 **/
export const Combinetz_ContentAttachmentConditionObj=async (): Promise<ConditionCollection> =>
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
const objtz_ContentAttachmentCond = new ConditionCollection();
let strWhereCond = " 1 = 1 ";
//如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
try
{
if ( idResourceType_q.value != "" && idResourceType_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clstz_ContentAttachmentEN.con_IdResourceType, idResourceType_q.value);
objtz_ContentAttachmentCond.SetCondFldValue(clstz_ContentAttachmentEN.con_IdResourceType, idResourceType_q.value, "=");
}

}
catch(objException)
{
const strMsg:string = Format("在组合查询条件对象(Combinetz_ContentAttachmentConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
export const Combinetz_ContentAttachmentConditionObj4ExportExcel = async ():Promise<ConditionCollection> => 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
const objtz_ContentAttachmentCond = new ConditionCollection();
...
{
const strMsg:string = Format("在组合导出Excel条件对象(Combinetz_ContentAttachmentConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objtz_ContentAttachmentCond.whereCond = strWhereCond;
return objtz_ContentAttachmentCond;
...
if (reftz_ContentAttachment_List.value != null) reftz_ContentAttachment_List.value.selectAllChecked = false;
};
