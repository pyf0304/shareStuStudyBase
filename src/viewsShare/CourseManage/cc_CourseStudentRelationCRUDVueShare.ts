...
export const Combinecc_CourseStudentRelationCondition = async (): Promise<string> => {
  ...
  try {
    if (courseId_q.value != '' && courseId_q.value != '0') {
      strWhereCond += Format(
        " And {0} = '{1}'",
        clscc_CourseStudentRelationEN.con_CourseId,
        courseId_q.value,
      );
    }
  } catch (objException) {
    const strMsg: string = Format(
      '在组合查询条件(Combinecc_CourseStudentRelationCondition)时出错!请联系管理员!{0}',
      objException,
    );
    throw strMsg;
  }
 ...
 * @returns 条件串(strWhereCond)
 **/
export const Combinecc_CourseStudentRelationConditionObj =
  async (): Promise<clscc_CourseStudentRelationEN> => {
    //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
    //例如 1 = 1 && UserName = '张三'
    const objcc_CourseStudentRelationCond = new clscc_CourseStudentRelationEN();
    let strWhereCond = ' 1 = 1 ';
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (courseId_q.value != '' && courseId_q.value != '0') {
        ...
        objcc_CourseStudentRelationCond.SetCondFldValue(
          clscc_CourseStudentRelationEN.con_CourseId,
          courseId_q.value,
          '=',
        );
      ...
      const strMsg: string = Format(
        '在组合查询条件对象(Combinecc_CourseStudentRelationConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
 ...
 * @returns 条件串(strWhereCond)
 **/
export const Combinecc_CourseStudentRelationConditionObj4ExportExcel =
  async (): Promise<clscc_CourseStudentRelationEN> => {
    //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
    //例如 1 = 1 && UserName = '张三'
    const objcc_CourseStudentRelationCond = new clscc_CourseStudentRelationENEx();
    let strWhereCond = ' 1 = 1 ';
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (courseId_q.value != '' && courseId_q.value != '0') {
      ...
      const strMsg: string = Format(
        '在组合导出Excel条件对象(Combinecc_CourseStudentRelationConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
