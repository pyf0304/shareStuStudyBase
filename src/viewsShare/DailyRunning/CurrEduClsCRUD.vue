<template>
  <div id="divLayout" ref="refDivLayout" class="div_layout">
    ...
    <div id="divList" ref="refDivList" class="div_List">
      <CurrEduCls_ListCom
        ref="refCurrEduCls_List"
        :items="dataListCurrEduCls"
...
<script lang="ts">
  ...
  import CurrEduCls_ListCom from '@/viewsShare/DailyRunning/CurrEduCls_List.vue';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const objPage = ref<CurrEduClsCRUDEx>();
      const objPage_Edit = ref<CurrEduCls_EditEx>();
      const opType = ref('');
      const thisConstructorName = 'CurrEduClsCRUD';
      ...
      const btnQuery_Click = async () => {
        ...
        await objPage.value.BindGv_CurrEduCls4Func(refDivList.value);
      };
      ...
      const btnCreate_Click = async () => {
        ...
        objPage_Edit.value = new CurrEduCls_EditEx('CurrEduCls_EditEx', objPage.value);
        ...
        try {
          opType.value = 'Add';
          const bolIsSuccess = await objPage_Edit.value.ShowDialog_CurrEduCls(opType.value);
      ...
      const btnUpdate_Click = async () => {
        ...
        const strIdCurrEduCls = GetFirstCheckedKeyIdInDivObj(divVarSet.refDivList);
        ...
        try {
          ...
          const bolIsSuccess = await objPage_Edit.value.ShowDialog_CurrEduCls(opType.value);
          if (bolIsSuccess == false) return;
          const update = await objPage_Edit.value.UpdateRecord(strIdCurrEduCls);
          if (update == false) {
      ...
      const btnDelete_Click = async () => {
        ...
        try {
          ...
          await objPage.value.BindGv_CurrEduCls4Func(divVarSet.refDivList);
        } catch (e) {
      ...
      const btnExportExcel_Click = async () => {
        ...
        const objExportExcelData: ExportExcelData = await objPage.value.ExportExcel_CurrEduCls();
      ...
      onMounted(() => {
        ...
        CurrEduClsCRUDEx.vuebtn_Click = btn_Click;
        CurrEduClsCRUDEx.GetPropValue = GetPropValue;
        objPage.value = new CurrEduClsCRUDEx();
      ...
      function btn_Click(strCommandName: string, strKeyId: string) {
        ...
        CurrEduClsCRUDEx.btn_Click(strCommandName, strKeyId);
      }
      return {
        showErrorMessage,
        dataListCurrEduCls,
    ...
    methods: {
      ...
      async EditTabRelaInfo(data: any) {
        ...
        router.push({ name: 'editCurrEduCls', params: { courseId: data.courseId } });
      },
      ...
      async SortColumn(data: any) {
        console.log('data:', data);
        const objPage = new CurrEduClsCRUDEx();
