/**
 * 类名:ge_StructureGraph_EditEx(界面:ge_StructureGraphCRUD,01120217)
 * 表名:ge_StructureGraph(01120895)
 * 版本:2025.02.27.1(服务器:WIN-SRV103-116)
 * 日期:2025/03/10 11:05:56
 * 生成者:
 工程名称:问卷调查(0112)
...
*/
export default class ge_StructureGraph_EditEx extends ge_StructureGraph_Edit {
  /**
 ...
 **/
  public static btnEdit_Click(strCommandName: string, strKeyId: string) {
    ...
    const objPage: ge_StructureGraph_EditEx = <ge_StructureGraph_EditEx>(
      ge_StructureGraph_Edit.GetPageEditObj('ge_StructureGraph_EditEx')
    );
    if (objPage == null) {
      const strMsg = `从预存编辑类获取关键字:[ge_StructureGraph_EditEx]的对象为空，请在调用编辑类(父类)的定义一下！`;
