<template>
  ...
  <el-dialog v-model="dialogVisible" :width="dialogWidth" :show-close="false">
    ...
    <template #footer>
      ...
      <el-button id="btnSubmitKnowledgeType" type="primary" @click="btnSubmit_Click">{{
        strSubmitButtonText
      }}</el-button>
    </template>
  </el-dialog>
...
<script lang="ts">
  ...
  import { CourseId_Session, refDivEdit } from '@/viewsShare/Knowledges/KnowledgeTypeVueShare';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const keyId = ref('');
      const objPage_Edit = ref<KnowledgeType_EditEx>();
      const dialogVisible = ref(false);
      const dialogWidth = ref('800px'); // 设置对话框的宽度
      const showDialog = async (pobjPage_Edit: KnowledgeType_EditEx) => {
