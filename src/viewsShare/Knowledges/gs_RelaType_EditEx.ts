/**
 * 类名:gs_RelaType_EditEx(界面:gs_RelaTypeCRUD)
 * 表名:gs_RelaType(01120871)
 * 版本:2023.08.27.1(服务器:WIN-SRV103-116)
 * 日期:2023/08/28 10:16:36
 * 生成者:
 工程名称:问卷调查(0112)
...
/* gs_RelaType_EditEx 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_EditCSEx_TS4TypeScript:GeneCode)
*/
export default class gs_RelaType_EditEx extends gs_RelaType_Edit {
  /**
 ...
 **/
  public static btnEdit_Click(strCommandName: string, strKeyId: string) {
    ...
    const objPage: gs_RelaType_EditEx = <gs_RelaType_EditEx>(
      gs_RelaType_Edit.GetPageEditObj('gs_RelaType_EditEx')
    );
    if (objPage == null) {
      const strMsg = `当前编辑页面没有按关键字:'gs_RelaType_EditEx'初始化过，请检查！`;
      alert(strMsg);
