<template>
  ...
  <el-dialog v-model="dialogVisible" :width="dialogWidth" :show-close="false">
      ...
      >
        <tbody>
          ...
          <tr id="trExamPaperTypeId">
            ...
            <td class="text-left">
              <select
                id="ddlExamPaperTypeId"
                v-model="examPaperTypeId"
                class="form-control form-control-sm"
                style="width: 150px"
              >
                <option
                  v-for="(item, index) in arrcc_ExamPaperType"
                  :key="index"
                  :value="item.examPaperTypeId"
                >
                  {{ item.examPaperTypeName }}
                </option></select
              >
            </td>
          </tr>
    ...
    <template #footer>
      ...
      <el-button id="btnSubmitcc_CourseExamPaper" type="primary" @click="btnSubmit_Click">{{
        strSubmitButtonText
      }}</el-button>
    </template>
  </el-dialog>
...
<script lang="ts">
  ...
  import cc_CourseExamPaper_EditEx from '@/viewsShare/Knowledges/cc_CourseExamPaper_EditEx';
  ...
  import { cc_ExamPaperType_GetArrcc_ExamPaperType } from '@/ts/L3ForWApi/SysPara/clscc_ExamPaperTypeWApi';
  import { XzMajor_GetArrXzMajor } from '@/ts/L3ForWApi/BaseInfo/clsXzMajorWApi';
  import { cc_PaperDispMode_GetArrcc_PaperDispMode } from '@/ts/L3ForWApi/Knowledges/clscc_PaperDispModeWApi';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const arrcc_ExamPaperType = ref<clscc_ExamPaperTypeEN[] | null>([]);
      const arrXzMajor = ref<clsXzMajorEN[] | null>([]);
      const arrcc_PaperDispMode = ref<clscc_PaperDispModeEN[] | null>([]);
      ...
      async function BindDdl4EditRegionInDiv() {
        ...
        arrcc_Course.value = await cc_Course_GetArrcc_Course(); //编辑区域
        courseId.value = '0';
        ...
        arrcc_ExamPaperType.value = await cc_ExamPaperType_GetArrcc_ExamPaperType(); //编辑区域
        examPaperTypeId.value = '0';

        arrXzMajor.value = await XzMajor_GetArrXzMajor(); //编辑区域
        idXzMajor.value = '0';

        arrcc_PaperDispMode.value = await cc_PaperDispMode_GetArrcc_PaperDispMode(); //编辑区域
      ...
      async function GetEditDatacc_CourseExamPaperObj() {
        ...
        pobjcc_CourseExamPaperEN.SetCourseId(Is0EqEmpty(courseId.value)); // 课程Id
        pobjcc_CourseExamPaperEN.SetCourseChapterId(Is0EqEmpty(courseChapterId.value)); // 课程章节
        pobjcc_CourseExamPaperEN.SetPaperIndex(Number(paperIndex.value)); // 试卷序号
        pobjcc_CourseExamPaperEN.SetExamPaperTypeId(Is0EqEmpty(examPaperTypeId.value)); // 试卷类型
        pobjcc_CourseExamPaperEN.SetPaperTime(Number(paperTime.value)); // 考试时间（分钟）
        pobjcc_CourseExamPaperEN.SetIdXzMajor(Is0EqEmpty(idXzMajor.value)); // 专业
        pobjcc_CourseExamPaperEN.SetPaperDispModeId(Is0EqEmpty(paperDispModeId.value)); // 试卷显示模式Id
      ...
      async function ShowDataFromcc_CourseExamPaperObj(
        ...
        courseId.value = IsNullOrEmptyEq0(pobjcc_CourseExamPaperEN.courseId); // 课程Id
        courseChapterId.value = IsNullOrEmptyEq0(pobjcc_CourseExamPaperEN.courseChapterId); // 课程章节
        paperIndex.value = pobjcc_CourseExamPaperEN.paperIndex; // 试卷序号
        examPaperTypeId.value = IsNullOrEmptyEq0(pobjcc_CourseExamPaperEN.examPaperTypeId); // 试卷类型
        paperTime.value = pobjcc_CourseExamPaperEN.paperTime; // 考试时间（分钟）
        idXzMajor.value = IsNullOrEmptyEq0(pobjcc_CourseExamPaperEN.idXzMajor); // 专业
        paperDispModeId.value = IsNullOrEmptyEq0(pobjcc_CourseExamPaperEN.paperDispModeId); // 试卷显示模式Id
      ...
      function Clear() {
        ...
        idXzMajor.value = '0';
        paperDispModeId.value = '0';
        examCreateTime.value = '';
        createUserID.value = '';
        isOpenToAllStu.value = false;
        isOpenToSchool.value = false;
        isOpenToSocial.value = false;
        isAutoGeneQuestion.value = false;
        isCanMultiDo.value = false;
        memo.value = '';
      }
      ...
      const keyId = ref('');
      const objPage_Edit = ref<cc_CourseExamPaper_EditEx>();
      const dialogVisible = ref(false);
      const dialogWidth = ref('800px'); // 设置对话框的宽度
      const showDialog = async (pobjPage_Edit: cc_CourseExamPaper_EditEx) => {
