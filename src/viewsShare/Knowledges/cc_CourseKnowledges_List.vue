      ...
      <table>
        <tbody>
          <tr class="text-primary">
            ...
            <th @click="sortColumn('questionNum|Ex')">
              题目数
              <span>
                <i
                  :class="
                    sortColumnKey === 'questionNum|Ex'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
            ...
            <th @click="sortColumn('knowledgeName')">
              知识点名称
              <span>
                <i
                  :class="
                    sortColumnKey === 'knowledgeName'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
            ...
            <th @click="sortColumn('knowledgeModuleName|Ex')">
              模块
              <span>
                <i
                  :class="
                    sortColumnKey === 'knowledgeModuleName|Ex'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
            ...
            <th @click="sortColumn('kpSourceName|Ex')">
              知识点来源名
              <span>
                <i
                  :class="
                    sortColumnKey === 'kpSourceName|Ex'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
            ...
            <th @click="sortColumn('courseChapterName|Ex')">
              章节名
              <span>
                <i
                  :class="
                    sortColumnKey === 'courseChapterName|Ex'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
            ...
            <th @click="sortColumn('knowledgeTypeName|Ex')">
              类型
              <span>
                <i
                  :class="
                    sortColumnKey === 'knowledgeTypeName|Ex'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
            ...
            <th v-if="showCourseColumn" @click="sortColumn('courseName|Ex')">
              课程
              <span>
                <i
                  :class="
            ...
            <th @click="sortColumn('likeCount')">
              喜欢度
              <span>
                <i
                  :class="
                    sortColumnKey === 'likeCount'
                      ? sortDirection === 'Asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                      : 'arrow-neutral'
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            ...
            <td v-html="item.questionNum"></td>

            <td v-html="item.knowledgeName" :title="item.courseKnowledgeId"></td>

            <td v-html="item.knowledgeModuleName"></td>
            <td v-html="item.kpSourceName"></td>

            <td v-html="item.courseChapterName"></td>

            <td v-html="item.userId"></td>

            <td v-html="item.knowledgeTypeName"></td>

            <td>
              <!-- <i v-if="item.isShow" class="fas fa-check text-success"></i>
            <i v-else class="fas fa-times text-danger"></i>
            <i :class="item.isShow ? 'fas fa-check text-success' : 'fas fa-times text-danger'"></i>
         -->
              <i v-if="item.inUse" class="layui-icon">&#xe605;</i>
              <i v-else class="layui-icon" style="color: red">&#x1006;</i>
            </td>

            <td v-if="showCourseColumn" v-html="item.courseName"></td>
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      watchEffect(() => {
        ...
        console.log('showCourseColumn:', showCourseColumn.value);
      });
      /**
       * 提交编辑
      ...
      const btnSubmitSel = (item: any) => {
        emit('on-submit-sel', {
          courseKnowledgeId: item.courseKnowledgeId,
          knowledgeId: item.courseKnowledgeId,
          content: '这是所选择的知识点Id',
        });
      };
