...
<script lang="ts">
  ...
  import { courseId_f } from '@/viewsShare/Knowledges/cc_CourseChapterVueShare';
  import { clsPubLocalStorage } from '@/ts/PubFun/clsPubLocalStorage';
  import { cc_CourseKnowledgesEx_ImportKPBySheet } from '@/ts/L3ForWApiExShare/Knowledges/clscc_CourseKnowledgesExWApi';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      //   }
      // };
      async function exportToExcelV2(uploadXls4KP: UploadXls4KP) {
        try {
          ...
          // link.href = url;
          // link.setAttribute('download', uploadXls4KP.fileName);
      ...
      async function importData() {
        ...
        // const uploadXls4KP: UploadXls4KP = {
      ...
      async function importDataV2() {
        ...
        await exportToExcelV2(uploadXls4KP);
        console.log('导入的数据:', jsonData);
        alert('数据导入成功！文件已经下载！');
      }
      ...
      async function ImportDataFromXlsSheet() {
        ...
        if (jsonData.length == 0) {
          const strMsg = `在表:${sheetName}中，记录数为0，不能导入，请检查！`;
          alert(strMsg);
        ...
        try {
          const intRecNum = await cc_CourseKnowledgesEx_ImportKPBySheet(uploadXls4KP);
          if (intRecNum >= 0) {
