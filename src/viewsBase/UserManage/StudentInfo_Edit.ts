 ...
 /**
 * 类名:StudentInfo_Edit(界面:StudentInfoCRUD,01120157)
 * 表名:StudentInfo(01120131)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/10 22:26:07
 * 生成者:
 工程名称:问卷调查(0112)
...
import { divVarSet,refStudentInfo_Edit } from "@/viewsShare/UserManage/StudentInfoVueShare";
...
{
 refStudentInfo_Edit.value.hideDialog();
}
}
...
if (StudentInfo_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
if (refStudentInfo_Edit.value == null)
{
const strMsg = Format(
...
await refStudentInfo_Edit.value.showDialog(this);
}
...
refStudentInfo_Edit.value.Clear();
...
try
{
objStudentInfoEN = await refStudentInfo_Edit.value.GetEditDataStudentInfoObj();
}
catch(e)
{
...
console.error(strMsg);
        ...
        //3、用提供的关键字初始化一个类对象；
refStudentInfo_Edit.value.ShowDataFromStudentInfoObj(objStudentInfoEN);
}
...
await refStudentInfo_Edit.value.ShowDataFromStudentInfoObj(objStudentInfoEN);
...
const objStudentInfoEN = await refStudentInfo_Edit.value.GetEditDataStudentInfoObj();
...
refStudentInfo_Edit.value.strCancelButtonText = value;
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmitStudentInfo():string {
const strValue = refStudentInfo_Edit.value.strSubmitButtonText;
return strValue;
}
 /**
...
refStudentInfo_Edit.value.strSubmitButtonText = value;
}
}