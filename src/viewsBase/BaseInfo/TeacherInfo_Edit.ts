/**
 * 类名:TeacherInfo_Edit(界面:TeacherInfoCRUD,01120122)
 * 表名:TeacherInfo(01120093)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/10 22:24:54
 * 生成者:
 工程名称:问卷调查(0112)
...
import { divVarSet, refTeacherInfo_Edit } from '@/viewsShare/BaseInfo/TeacherInfoVueShare';
...
export abstract class TeacherInfo_Edit {
  ...
  public HideDialog_TeacherInfo() {
    if (TeacherInfo_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      refTeacherInfo_Edit.value.hideDialog();
    }
  }
  ...
  public async ShowDialog_TeacherInfo(strOp: string): Promise<boolean> {
    ...
    if (TeacherInfo_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (refTeacherInfo_Edit.value == null) {
        const strMsg = Format(
      ...
      await refTeacherInfo_Edit.value.showDialog(this);
    }
   ...
   **/
  public async btnUpdateRecordInTab_Click(strIdTeacher: string) {
    ...
    try {
      ...
      const bolIsSuccess = await this.ShowDialog_TeacherInfo(this.opType);
      if (bolIsSuccess == false) return;
      this.UpdateRecord(strIdTeacher);
    } catch (e) {
      const strMsg = Format(
   ...
   **/
  public async btnUpdateRecord_Click(strIdTeacher: string) {
    ...
    try {
      ...
      const update = await this.UpdateRecord(strIdTeacher);
      if (update == false) {
  ...
  public async AddNewRecord() {
    ...
    refTeacherInfo_Edit.value.Clear();
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    let objTeacherInfoEN;
    try {
      objTeacherInfoEN = await refTeacherInfo_Edit.value.GetEditDataTeacherInfoObj();
    } catch (e) {
      const strMsg = Format(
        '从界面获取数据不成功,{0}.(in {1}.{2})',
        e,
    ...
    try {
      ...
      if (returnBool == true) {
        const strInfo = `添加[教师(TeacherInfo)]记录成功!`;
        //显示信息框
  ...
  public async ShowData(strIdTeacher: string) {
    ...
    //3、用提供的关键字初始化一个类对象；
    refTeacherInfo_Edit.value.ShowDataFromTeacherInfoObj(objTeacherInfoEN);
  }
  ...
  public async UpdateRecord(strIdTeacher: string): Promise<boolean> {
    ...
    try {
      ...
      await refTeacherInfo_Edit.value.ShowDataFromTeacherInfoObj(objTeacherInfoEN);
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    const objTeacherInfoEN = await refTeacherInfo_Edit.value.GetEditDataTeacherInfoObj();
  ...
  public async EditRecordExSave(): Promise<boolean> {
    ...
    try {
      ...
      const returnBool = await TeacherInfo_EditRecordExAsync(objTeacherInfoEN);
      if (returnBool == true) {
        const teacherInfoStore = useTeacherInfoStore();
        await teacherInfoStore.delObj(objTeacherInfoEN.idTeacher);
      }
  ...
  public set btnCancelTeacherInfo(value: string) {
    refTeacherInfo_Edit.value.strCancelButtonText = value;
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitTeacherInfo(): string {
    const strValue = refTeacherInfo_Edit.value.strSubmitButtonText;
    return strValue;
  }
  /**
  ...
  public set btnSubmitTeacherInfo(value: string) {
    refTeacherInfo_Edit.value.strSubmitButtonText = value;
  }
}
