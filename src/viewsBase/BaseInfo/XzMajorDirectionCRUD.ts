 ...
 /**
 * 类名:XzMajorDirectionCRUD(界面:XzMajorDirectionCRUD,01120090)
 * 表名:XzMajorDirection(01120552)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/10 22:24:16
 * 生成者:
 工程名称:问卷调查(0112)
...
import { IdXzMajor_Cache,CombineXzMajorDirectionConditionObj4ExportExcel,CombineXzMajorDirectionConditionObj,divVarSet,viewVarSet,dataColumn,BindTabByList,refvXzMajorDirection_List } from "@/viewsShare/BaseInfo/XzMajorDirectionVueShare";
import { XzMajorDirection_GetRecCountByCondCache,XzMajorDirection_GetSubObjLstCache,XzMajorDirection_DelRecordAsync,XzMajorDirection_ReFreshCache,XzMajorDirection_GetObjByMajorDirectionIdAsync,XzMajorDirection_FuncMapByFldName,XzMajorDirection_GetObjExLstByPagerCache,XzMajorDirection_GetObjLstByMajorDirectionIdLstAsync,XzMajorDirection_GetMaxStrIdAsync,XzMajorDirection_AddNewRecordAsync,XzMajorDirection_DelXzMajorDirectionsAsync } from "@/ts/L3ForWApi/BaseInfo/clsXzMajorDirectionWApi";
...
if (viewVarSet.sortXzMajorDirectionBy == '') viewVarSet.sortXzMajorDirectionBy = `${ clsXzMajorDirectionEN.con_MajorDirectionId } Asc`;
//2、显示无条件的表内容在GridView中
await this.BindGv_XzMajorDirection4Func(divVarSet.refDivList);
}
catch (e)
{
 ...
 **/
public CombineData(arrXzMajorDirectionObjLst: Array<clsXzMajorDirectionEN>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
...
const strFileName = Format("专业方向({0})导出.xlsx",
 clsXzMajorDirectionEN._CurrTabName);
const strSheetName = "专业方向列表";
 ...
 **/
public async ExportExcel_XzMajorDirectionCache(): Promise<ExportExcelData> 
{
const strThisFuncName = this.ExportExcel_XzMajorDirectionCache.name;
if (viewVarSet.sortXzMajorDirectionBy == null)
{
const strMsg = Format("在显示列表时,排序字段(sortXzMajorDirectionBy)为空,请检查!(In BindGv_XzMajorDirectionCache)");
console.error(strMsg);
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
}
const objXzMajorDirectionCond = await CombineXzMajorDirectionConditionObj4ExportExcel();
objXzMajorDirectionCond.SetCondFldValue(clsXzMajorDirectionEN.con_IdXzMajor, IdXzMajor_Cache.value, "=");
 let arrXzMajorDirectionObjLst: Array <clsXzMajorDirectionEN> = [];
try
{

this.recCount = await XzMajorDirection_GetRecCountByCondCache(objXzMajorDirectionCond ,IdXzMajor_Cache.value);
if (this.recCount == 0)
{
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objXzMajorDirectionCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
...
arrXzMajorDirectionObjLst = await XzMajorDirection_GetSubObjLstCache(objXzMajorDirectionCond ,IdXzMajor_Cache.value);
}
catch(e)
{
...
{
const strKey = Format("{0}_{1}", clsXzMajorDirectionEN._CurrTabName, IdXzMajor_Cache.value);
...
arrXzMajorDirectionObjLst = arrXzMajorDirectionObjLst.sort(this.SortFunExportExcel);
 return this.CombineData(arrXzMajorDirectionObjLst, arrDataColumn); 
 ...
 **/
public async BindTab_XzMajorDirection(divContainer: HTMLDivElement, arrXzMajorDirectionExObjLst: Array<clsXzMajorDirectionENEx>) 
{
...
if (refvXzMajorDirection_List.value != null){
dataColumn.value = arrDataColumn;
await BindTabByList(arrXzMajorDirectionExObjLst, this.dispAllErrMsg_q); 
}
else {
const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
if (divDataLst == null)
{
alert("在BindTab_XzMajorDirection函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrXzMajorDirectionExObjLst, arrDataColumn,  clsXzMajorDirectionEN.con_MajorDirectionId, this); 
}
 ...
 **/
public async BindTab_XzMajorDirection4Func(divContainer: HTMLDivElement, arrXzMajorDirectionExObjLst: Array<clsXzMajorDirectionENEx>) 
{
...
if (divDataLst == null)
{
alert("在BindTab_XzMajorDirection4Func函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrXzMajorDirectionExObjLst, arrDataColumn,  clsXzMajorDirectionEN.con_MajorDirectionId, this); 
}
 ...
 **/
public async ExtendFldFuncMap(arrXzMajorDirectionExObjLst: Array<clsXzMajorDirectionENEx>, arrDataColumn: Array<clsDataColumn>) {
...
await XzMajorDirection_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
await this.BindGv_XzMajorDirection4Func(this.listPara.listDiv);
}
...
const objXzMajorDirectionCond = await CombineXzMajorDirectionConditionObj();
objXzMajorDirectionCond.SetCondFldValue(clsXzMajorDirectionEN.con_IdXzMajor, IdXzMajor_Cache.value, "=");
...
lblMsg.innerHTML = Format("根据条件:[{0}]获取的对象列表数为0!", objXzMajorDirectionCond.whereCond);
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objXzMajorDirectionCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
BindTabByList(arrXzMajorDirectionExObjLst, true);
return;
}
...
{
strSortFun = XzMajorDirectionCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
}
const objPagerPara: stuPagerPara = {
pageIndex: intCurrPageIndex,
pageSize: this.pageSize,
whereCond: strWhereCond,
conditionCollection: objXzMajorDirectionCond,
orderBy: viewVarSet.sortXzMajorDirectionBy,
sortFun: strSortFun
}
arrXzMajorDirectionExObjLst = await XzMajorDirection_GetObjExLstByPagerCache(objPagerPara ,IdXzMajor_Cache.value);
}
catch(e)
{
const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, viewVarSet.ascOrDesc4SortFun, viewVarSet.sortXzMajorDirectionBy, strSortExpress);
// 将 sortBy 按空格分成两部分
const [sortColumnKey, sortDirection] = sortBy.split(' ');
if (Object.prototype.hasOwnProperty.call(clsXzMajorDirectionENEx, sortColumnKey))
{
// 调用 SortColumn 函数
this.SortColumn(sortColumnKey, sortDirection);
return;
}
viewVarSet.sortXzMajorDirectionBy = sortBy;
...
try
{
const arrXzMajorDirectionObjLst = await XzMajorDirection_GetObjLstByMajorDirectionIdLstAsync(arrMajorDirectionId);
 ...
 **/
public ShowXzMajorDirectionObj(divContainer: HTMLDivElement, objXzMajorDirection: clsXzMajorDirectionEN) 
{
if (divContainer == null)
{
