 ...
 /**
 * 类名:TeacherInfoCRUD(界面:TeacherInfoCRUD,01120122)
 * 表名:TeacherInfo(01120093)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/10 22:24:50
 * 生成者:
 工程名称:问卷调查(0112)
...
import { CombineTeacherInfoCondition,divVarSet,viewVarSet,dataColumn,BindTabByList,refTeacherInfo_List } from "@/viewsShare/BaseInfo/TeacherInfoVueShare";
import { TeacherInfo_GetRecCountByCondAsync,TeacherInfo_GetObjLstAsync,TeacherInfo_DelRecordAsync,TeacherInfo_GetObjByIdTeacherAsync,TeacherInfo_FuncMapByFldName,TeacherInfo_GetObjLstByIdTeacherLstAsync,TeacherInfo_GetMaxStrIdAsync,TeacherInfo_AddNewRecordAsync,TeacherInfo_UpdateRecordAsync,TeacherInfo_DelTeacherInfosAsync } from "@/ts/L3ForWApi/BaseInfo/clsTeacherInfoWApi";
import { TeacherInfoEx_CopyToEx,TeacherInfoEx_GetObjExLstByPagerAsync } from "@/ts/L3ForWApiExShare/BaseInfo/clsTeacherInfoExWApi";
...
if (viewVarSet.sortTeacherInfoBy == '') viewVarSet.sortTeacherInfoBy = `${ clsTeacherInfoEN.con_TeacherName } Asc`;
 ...
 **/
public CombineData(arrTeacherInfoObjLst: Array<clsTeacherInfoEN>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
...
const strFileName = Format("教师({0})导出.xlsx",
 clsTeacherInfoEN._CurrTabName);
const strSheetName = "教师列表";
 ...
 **/
public CombineData4Func(arrTeacherInfoExObjLst: Array<clsTeacherInfoENEx>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
 ...
 **/
public async ExportExcel_TeacherInfo4Func(): Promise<ExportExcelData> 
{
const strThisFuncName = this.ExportExcel_TeacherInfo4Func.name;
if (viewVarSet.sortTeacherInfoBy == null)
{
const strMsg = Format("在显示列表时,排序字段(sortTeacherInfoBy)为空,请检查!(In BindGv_TeacherInfoCache)");
console.error(strMsg);
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
}

const strWhereCond = await CombineTeacherInfoCondition();
...
const arrTeacherInfoObjLst = await TeacherInfo_GetObjLstAsync(strWhereCond);
...
arrTeacherInfoExObjLst = arrTeacherInfoExObjLst.sort(this.SortFunExportExcel);
 return this.CombineData(arrTeacherInfoExObjLst, arrDataColumn); 
 ...
 **/
public async BindTab_TeacherInfo(divContainer: HTMLDivElement, arrTeacherInfoExObjLst: Array<clsTeacherInfoENEx>) 
{
...
{
fldName: clsTeacherInfoENEx.con_SchoolNameA,
sortBy: "schoolNameA",
sortFun: SortFun,
getDataSource: "",
colHeader: "学校",
...
if (refTeacherInfo_List.value != null){
dataColumn.value = arrDataColumn;
await BindTabByList(arrTeacherInfoExObjLst, this.dispAllErrMsg_q); 
}
else {
const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
if (divDataLst == null)
{
alert("在BindTab_TeacherInfo函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrTeacherInfoExObjLst, arrDataColumn,  clsTeacherInfoEN.con_IdTeacher, this); 
}
 ...
 **/
public async BindTab_TeacherInfo4Func(divContainer: HTMLDivElement, arrTeacherInfoExObjLst: Array<clsTeacherInfoENEx>) 
{
...
if (divDataLst == null)
{
alert("在BindTab_TeacherInfo4Func函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrTeacherInfoExObjLst, arrDataColumn,  clsTeacherInfoEN.con_IdTeacher, this); 
}
 ...
 **/
public async ExtendFldFuncMap(arrTeacherInfoExObjLst: Array<clsTeacherInfoENEx>, arrDataColumn: Array<clsDataColumn>) {
const arrFldName = clsTeacherInfoEN.AttributeName;
for (const objDataColumn of arrDataColumn) {
if (IsNullOrEmpty(objDataColumn.fldName) == true) continue;
if (arrFldName.indexOf(objDataColumn.fldName) > -1) continue;
for (const objInFor of arrTeacherInfoExObjLst) {
await TeacherInfo_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
//console.trace();
alert(strMsg);
BindTabByList(arrTeacherInfoExObjLst, true);
return;
}
...
orderBy: viewVarSet.sortTeacherInfoBy,//如果该字段为空,就使用下面的排序函数
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, viewVarSet.ascOrDesc4SortFun, viewVarSet.sortTeacherInfoBy, strSortExpress);
// 将 sortBy 按空格分成两部分
const [sortColumnKey, sortDirection] = sortBy.split(' ');
if (Object.prototype.hasOwnProperty.call(clsTeacherInfoENEx, sortColumnKey))
{
// 调用 SortColumn 函数
 ...
 **/
public async SetIdSchool(arrIdTeacher: Array<string>, strIdSchool: string)
{
...
ObjectAssign(objTeacherInfoEN, objInFor);
objTeacherInfoEN.SetIdTeacher(objInFor.idTeacher);
objTeacherInfoEN.SetIdSchool(strIdSchool);
let returnBool = false;
try
{
    objTeacherInfoEN.sfUpdFldSetStr = objTeacherInfoEN.updFldString;//设置哪些字段被修改(脏字段)
returnBool = await TeacherInfo_UpdateRecordAsync(objTeacherInfoEN);
}
catch (e)
{
const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
...
{
const teacherInfoStore = useTeacherInfoStore();
arrIdTeacher.forEach(async (strIdTeacher) => {
await teacherInfoStore.delObj(strIdTeacher);
});
}
}
