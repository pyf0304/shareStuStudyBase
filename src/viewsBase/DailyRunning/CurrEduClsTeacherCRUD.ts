 ...
 /**
 * 类名:CurrEduClsTeacherCRUD(界面:CurrEduClsTeacherCRUD,01120119)
 * 表名:CurrEduClsTeacher(01120124)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/10 20:09:01
 * 生成者:
 工程名称:问卷调查(0112)
...
import { CombineCurrEduClsTeacherCondition,IdSchool_Session,CourseId_Session,divVarSet,viewVarSet,dataColumn,BindTabByList,refCurrEduClsTeacher_List } from "@/viewsShare/DailyRunning/CurrEduClsTeacherVueShare";
import { CurrEduClsTeacher_GetRecCountByCondAsync,CurrEduClsTeacher_GetObjLstAsync,CurrEduClsTeacher_DelRecordAsync,CurrEduClsTeacher_GetObjByIdEduClsTeacherAsync,CurrEduClsTeacher_FuncMapByFldName,CurrEduClsTeacher_GetObjLstByIdEduClsTeacherLstAsync,CurrEduClsTeacher_AddNewRecordAsync,CurrEduClsTeacher_DelCurrEduClsTeachersAsync } from "@/ts/L3ForWApi/DailyRunning/clsCurrEduClsTeacherWApi";
import { CurrEduClsTeacherEx_CopyToEx,CurrEduClsTeacherEx_GetObjExLstByPagerAsync } from "@/ts/L3ForWApiExShare/DailyRunning/clsCurrEduClsTeacherExWApi";
...
if (viewVarSet.sortCurrEduClsTeacherBy == '') viewVarSet.sortCurrEduClsTeacherBy = `${ clsCurrEduClsTeacherEN.con_IdCurrEduCls } Asc`;
//2、显示无条件的表内容在GridView中
await this.BindGv_CurrEduClsTeacher4Func(divVarSet.refDivList);
}
catch (e)
{
 ...
 **/
public CombineData(arrCurrEduClsTeacherObjLst: Array<clsCurrEduClsTeacherEN>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
const intRowNum = arrCurrEduClsTeacherObjLst.length;
const intColNum = arrDataColumn.length;
const arrData: Array <Record<string, any>> = [];
for (let i = 0; i < intRowNum; i++)
{
const objEN: clsCurrEduClsTeacherEN = arrCurrEduClsTeacherObjLst[i];
...
const strFileName = Format("当前教学班教师({0})导出.xlsx",
 clsCurrEduClsTeacherEN._CurrTabName);
const strSheetName = "当前教学班教师列表";
 ...
 **/
public CombineData4Func(arrCurrEduClsTeacherExObjLst: Array<clsCurrEduClsTeacherENEx>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
const intRowNum = arrCurrEduClsTeacherExObjLst.length;
const intColNum = arrDataColumn.length;
const arrData: Array <Record<string, any>> = [];
for (let i = 0; i < intRowNum; i++)
{
const objEN: clsCurrEduClsTeacherENEx = arrCurrEduClsTeacherExObjLst[i];
const objRow: Record<string, any> = { };
for (let j = 0; j < intColNum; j++)
{
const fldName = arrDataColumn[j].fldName;
 ...
 **/
public async ExportExcel_CurrEduClsTeacher4Func(): Promise<ExportExcelData> 
{
const strThisFuncName = this.ExportExcel_CurrEduClsTeacher4Func.name;
if (viewVarSet.sortCurrEduClsTeacherBy == null)
{
const strMsg = Format("在显示列表时,排序字段(sortCurrEduClsTeacherBy)为空,请检查!(In BindGv_CurrEduClsTeacherCache)");
console.error(strMsg);
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
}

const strWhereCond = await CombineCurrEduClsTeacherCondition();
...
const arrCurrEduClsTeacherObjLst = await CurrEduClsTeacher_GetObjLstAsync(strWhereCond);
arrCurrEduClsTeacherExObjLst = arrCurrEduClsTeacherObjLst.map(CurrEduClsTeacherEx_CopyToEx);
}
catch(e)
{
...
{
const strKey = Format("{0}", clsCurrEduClsTeacherEN._CurrTabName);
...
arrCurrEduClsTeacherExObjLst = arrCurrEduClsTeacherExObjLst.sort(this.SortFunExportExcel);
 return this.CombineData(arrCurrEduClsTeacherExObjLst, arrDataColumn); 
//console.log("完成BindGv_CurrEduClsTeacher!");
}
catch(e)
{
...
Redirect("/Index/Main_CurrEduClsTeacher");
}
catch(e)
{
...
{
await this.ExportExcel_CurrEduClsTeacher4Func();
}

 /** 显示CurrEduClsTeacher对象的所有属性值
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
 * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
 * @param arrCurrEduClsTeacherObjLst:需要绑定的对象列表
 **/
public async BindTab_CurrEduClsTeacher(divContainer: HTMLDivElement, arrCurrEduClsTeacherExObjLst: Array<clsCurrEduClsTeacherENEx>) 
{
...
if (divDataLst == null)
{
alert("在BindTab_CurrEduClsTeacher函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrCurrEduClsTeacherExObjLst, arrDataColumn,  clsCurrEduClsTeacherEN.con_IdEduClsTeacher, this); 
}
 ...
 * @param arrCurrEduClsTeacherExObjLst:需要绑定的对象列表
 **/
public async BindTab_CurrEduClsTeacher4Func(divContainer: HTMLDivElement, arrCurrEduClsTeacherExObjLst: Array<clsCurrEduClsTeacherENEx>) 
{
...
if (divDataLst == null)
{
alert("在BindTab_CurrEduClsTeacher4Func函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrCurrEduClsTeacherExObjLst, arrDataColumn,  clsCurrEduClsTeacherEN.con_IdEduClsTeacher, this); 
}
 ...
 * @param arrCurrEduClsTeacherExObjLst:需要映射的对象列表
 * @param arrDataColumn:用于绑定表的数据列信息
 **/
public async ExtendFldFuncMap(arrCurrEduClsTeacherExObjLst: Array<clsCurrEduClsTeacherENEx>, arrDataColumn: Array<clsDataColumn>) {
...
objInFor.idSchool = IdSchool_Session.value;
objInFor.courseId = CourseId_Session.value;
objInFor.courseId = CourseId_Session.value;
objInFor.courseId = CourseId_Session.value;
objInFor.courseId = CourseId_Session.value;
await CurrEduClsTeacher_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
//console.trace();
alert(strMsg);
BindTabByList(arrCurrEduClsTeacherExObjLst, true);
return;
}
...
orderBy: viewVarSet.sortCurrEduClsTeacherBy,//如果该字段为空,就使用下面的排序函数
sortFun: (x, y) => { console.log(x,y);return 0;}
}
arrCurrEduClsTeacherExObjLst = await CurrEduClsTeacherEx_GetObjExLstByPagerAsync(objPagerPara);
}
catch(e)
{
 ...
 **/
 public CopyToEx(objCurrEduClsTeacherENS:clsCurrEduClsTeacherEN ): clsCurrEduClsTeacherENEx
{
 ...
 * @returns 返回两个对象比较的结果
 **/
public SortFunExportExcel(a:clsCurrEduClsTeacherEN , b:clsCurrEduClsTeacherEN): number 
{
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, viewVarSet.ascOrDesc4SortFun, viewVarSet.sortCurrEduClsTeacherBy, strSortExpress);
// 将 sortBy 按空格分成两部分
const [sortColumnKey, sortDirection] = sortBy.split(' ');
if (Object.prototype.hasOwnProperty.call(clsCurrEduClsTeacherENEx, sortColumnKey))
{
// 调用 SortColumn 函数
this.SortColumn(sortColumnKey, sortDirection);
return;
}
viewVarSet.sortCurrEduClsTeacherBy = sortBy;
...
try
{
const arrCurrEduClsTeacherObjLst = await CurrEduClsTeacher_GetObjLstByIdEduClsTeacherLstAsync(arrIdEduClsTeacher);
 ...
 **/
public ShowCurrEduClsTeacherObj(divContainer: HTMLDivElement, objCurrEduClsTeacher: clsCurrEduClsTeacherEN) 
{
if (divContainer == null)
{
