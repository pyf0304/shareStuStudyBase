 ...
 /**
 * 类名:CurrEduClsCRUD(界面:CurrEduClsCRUD,01120118)
 * 表名:CurrEduCls(01120123)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/10 22:24:34
 * 生成者:
 工程名称:问卷调查(0112)
...
import { CombineCurrEduClsCondition,divVarSet,viewVarSet,dataColumn,BindTabByList,refCurrEduCls_List } from "@/viewsShare/DailyRunning/CurrEduClsVueShare";
import { CurrEduCls_GetRecCountByCondAsync,CurrEduCls_GetObjLstAsync,CurrEduCls_DelRecordAsync,CurrEduCls_GetObjByIdCurrEduClsAsync,CurrEduCls_FuncMapByFldName,CurrEduCls_GetObjLstByIdCurrEduClsLstAsync,CurrEduCls_GetMaxStrIdAsync,CurrEduCls_AddNewRecordAsync,CurrEduCls_DelCurrEduClssAsync } from "@/ts/L3ForWApi/DailyRunning/clsCurrEduClsWApi";
...
if (viewVarSet.sortCurrEduClsBy == '') viewVarSet.sortCurrEduClsBy = `${ clsCurrEduClsEN.con_CurrEduClsId } Asc`;
 ...
 **/
public CombineData(arrCurrEduClsObjLst: Array<clsCurrEduClsEN>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
...
const strFileName = Format("当前教学班({0})导出.xlsx",
 clsCurrEduClsEN._CurrTabName);
const strSheetName = "当前教学班列表";
 ...
 **/
public async ExportExcel_CurrEduCls(): Promise<ExportExcelData> 
{
const strThisFuncName = this.ExportExcel_CurrEduCls.name;
if (viewVarSet.sortCurrEduClsBy == null)
{
const strMsg = Format("在显示列表时,排序字段(sortCurrEduClsBy)为空,请检查!(In BindGv_CurrEduClsCache)");
console.error(strMsg);
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
}

const strWhereCond = await CombineCurrEduClsCondition();
...
arrCurrEduClsObjLst = arrCurrEduClsObjLst.sort(this.SortFunExportExcel);
  return this.CombineData(arrCurrEduClsObjLst, arrDataColumn); 
...
{
await this.ExportExcel_CurrEduCls();
}
 ...
 **/
public async BindTab_CurrEduCls(divContainer: HTMLDivElement, arrCurrEduClsExObjLst: Array<clsCurrEduClsENEx>) 
{
...
{
fldName: clsCurrEduClsENEx.con_UserTypeNameSim,
...
{
fldName: clsCurrEduClsENEx.con_LastVisitedDate,
sortBy: clsCurrEduClsENEx.con_LastVisitedDate,
sortFun: SortFun,
getDataSource: "",
colHeader: "修改日期",
text: "",
...
if (refCurrEduCls_List.value != null){
dataColumn.value = arrDataColumn;
await BindTabByList(arrCurrEduClsExObjLst, this.dispAllErrMsg_q); 
}
else {
const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
if (divDataLst == null)
{
alert("在BindTab_CurrEduCls函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrCurrEduClsExObjLst, arrDataColumn,  clsCurrEduClsEN.con_IdCurrEduCls, this); 
}
 ...
 **/
public async BindTab_CurrEduCls4Func(divContainer: HTMLDivElement, arrCurrEduClsExObjLst: Array<clsCurrEduClsENEx>) 
{
...
if (divDataLst == null)
{
alert("在BindTab_CurrEduCls4Func函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrCurrEduClsExObjLst, arrDataColumn,  clsCurrEduClsEN.con_IdCurrEduCls, this); 
}
 ...
 **/
public async ExtendFldFuncMap(arrCurrEduClsExObjLst: Array<clsCurrEduClsENEx>, arrDataColumn: Array<clsDataColumn>) {
const arrFldName = clsCurrEduClsEN.AttributeName;
for (const objDataColumn of arrDataColumn) {
if (IsNullOrEmpty(objDataColumn.fldName) == true) continue;
if (arrFldName.indexOf(objDataColumn.fldName) > -1) continue;
for (const objInFor of arrCurrEduClsExObjLst) {
await CurrEduCls_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
await this.BindGv_CurrEduCls4Func(this.listPara.listDiv);
}
...
//console.trace();
alert(strMsg);
BindTabByList(arrCurrEduClsExObjLst, true);
return;
}
...
{
const strKey = Format("{0}", clsCurrEduClsEN._CurrTabName);
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, viewVarSet.ascOrDesc4SortFun, viewVarSet.sortCurrEduClsBy, strSortExpress);
// 将 sortBy 按空格分成两部分
const [sortColumnKey, sortDirection] = sortBy.split(' ');
if (Object.prototype.hasOwnProperty.call(clsCurrEduClsENEx, sortColumnKey))
{
// 调用 SortColumn 函数
