 ...
 /**
 * 类名:cc_Course_Edit(界面:cc_CourseCRUD,01120072)
 * 表名:cc_Course(01120056)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/10 22:24:02
 * 生成者:
 工程名称:问卷调查(0112)
...
import { cc_Course_GetMaxStrIdAsync,cc_Course_CheckPropertyNew,cc_Course_AddNewRecordWithMaxIdAsync,cc_Course_ReFreshCache,cc_Course_GetUniCondStr,cc_Course_IsExistRecordAsync,cc_Course_GetUniCondStr4Update,cc_Course_IsExistAsync,cc_Course_GetObjByCourseIdAsync,cc_Course_CheckProperty4Update,cc_Course_UpdateRecordAsync,cc_Course_EditRecordExAsync } from "@/ts/L3ForWApi/CourseLearning/clscc_CourseWApi";
import { clscc_CourseEN } from "@/ts/L0Entity/CourseLearning/clscc_CourseEN";
import { cc_Course_DeleteKeyIdCache,divVarSet,refcc_Course_Edit } from "@/viewsShare/CourseLearning/cc_CourseVueShare";
...
{
 refcc_Course_Edit.value.hideDialog();
}
}
...
if (cc_Course_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
if (refcc_Course_Edit.value == null)
{
const strMsg = Format(
...
await refcc_Course_Edit.value.showDialog(this);
}
 ...
 **/
public async btnUpdateRecordInTab_Click(strCourseId: string) {
 ...
 **/
public async btnUpdateRecord_Click(strCourseId: string) {
...
const update = await this.UpdateRecord(strCourseId);
if (update == false)
{
...
if (this.iShowList != null) this.iShowList.BindGvCache(clscc_CourseEN._CurrTabName, returnKeyId);
}
}
else
{
...
if (this.iShowList != null) this.iShowList.BindGvCache(clscc_CourseEN._CurrTabName, this.keyId);
}
}
break;
case "确认修改":
...
const strInfo = `添加[课程(cc_Course)]记录成功!`;
//显示信息框
...
console.error(strMsg);
        ...
        //3、用提供的关键字初始化一个类对象；
refcc_Course_Edit.value.ShowDataFromcc_CourseObj(objcc_CourseEN);
}
...
await refcc_Course_Edit.value.ShowDataFromcc_CourseObj(objcc_CourseEN);
...
{
cc_Course_ReFreshCache();
cc_Course_DeleteKeyIdCache(this.keyId);
}
return returnBool;
}
catch(e)
...
const returnBool = await cc_Course_EditRecordExAsync(objcc_CourseEN);
if (returnBool == true)
{
cc_Course_ReFreshCache();
}
...
refcc_Course_Edit.value.strCancelButtonText = value;
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmitcc_Course():string {
const strValue = refcc_Course_Edit.value.strSubmitButtonText;
return strValue;
}
 /**
...
refcc_Course_Edit.value.strSubmitButtonText = value;
}
}