 ...
 /**
 * 类名:CacheUseStateCRUD(界面:CacheUseStateCRUD,01120263)
 * 表名:CacheUseState(01120689)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/11 00:33:42
 * 生成者:
 工程名称:问卷调查(0112)
...
import { UserId_Local,CombineCacheUseStateConditionObj4ExportExcel,CombineCacheUseStateConditionObj,divVarSet,viewVarSet,dataColumn,BindTabByList,refCacheUseState_List } from "@/views/SystemSet/CacheUseStateVueShare";
import { CacheUseState_GetRecCountByCondCache,CacheUseState_GetSubObjLstCache,CacheUseState_DelRecordAsync,CacheUseState_ReFreshCache,CacheUseState_GetObjBymIdAsync,CacheUseState_FuncMapByFldName,CacheUseState_GetObjExLstByPagerCache,CacheUseState_GetObjLstBymIdLstAsync,CacheUseState_AddNewRecordAsync,CacheUseState_DelCacheUseStatesAsync } from "@/ts/L3ForWApi/SystemSet/clsCacheUseStateWApi";
import { CacheUseStateEx_CopyToEx } from "@/ts/L3ForWApiExShare/SystemSet/clsCacheUseStateExWApi";
...
if (viewVarSet.sortCacheUseStateBy == '') viewVarSet.sortCacheUseStateBy = `${ clsCacheUseStateEN.con_mId } Asc`;
 ...
 **/
public CombineData(arrCacheUseStateObjLst: Array<clsCacheUseStateEN>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
...
const strFileName = Format("缓存使用状态({0})导出.xlsx",
 ...
 **/
public CombineData4Func(arrCacheUseStateExObjLst: Array<clsCacheUseStateENEx>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
...
{
const strMsg = Format("在显示列表时,排序字段(sortCacheUseStateBy)为空,请检查!(In BindGv_CacheUseStateCache)");
...
this.recCount = await CacheUseState_GetRecCountByCondCache(objCacheUseStateCond ,UserId_Local.value);
if (this.recCount == 0)
{
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objCacheUseStateCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
...
const arrCacheUseStateObjLst = await CacheUseState_GetSubObjLstCache(objCacheUseStateCond ,UserId_Local.value);
 ...
 **/
public async BindTab_CacheUseState(divContainer: HTMLDivElement, arrCacheUseStateExObjLst: Array<clsCacheUseStateENEx>) 
{
...
{
fldName: clsCacheUseStateENEx.con_CacheModeENName,
sortBy: "cacheModeENName",
sortFun: SortFun,
getDataSource: "",
colHeader: "缓存方式英文名",
text: "",
...
await BindTab(divDataLst, arrCacheUseStateExObjLst, arrDataColumn,  clsCacheUseStateEN.con_mId, this); 
}
 ...
 **/
public async BindTab_CacheUseState4Func(divContainer: HTMLDivElement, arrCacheUseStateExObjLst: Array<clsCacheUseStateENEx>) 
{
 ...
 **/
public async ExtendFldFuncMap(arrCacheUseStateExObjLst: Array<clsCacheUseStateENEx>, arrDataColumn: Array<clsDataColumn>) {
...
await CacheUseState_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
lblMsg.innerHTML = Format("根据条件:[{0}]获取的对象列表数为0!", objCacheUseStateCond.whereCond);
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objCacheUseStateCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
...
arrCacheUseStateExObjLst = await CacheUseState_GetObjExLstByPagerCache(objPagerPara ,UserId_Local.value);
}
catch(e)
{
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, viewVarSet.ascOrDesc4SortFun, viewVarSet.sortCacheUseStateBy, strSortExpress);
// 将 sortBy 按空格分成两部分
