...
import { CourseId_Session,Combinege_StructureGraphConditionObj4ExportExcel,Combinege_StructureGraphConditionObj,divVarSet,viewVarSet,dataColumn,BindTabByList,refge_StructureGraph_List } from "@/viewsShare/Knowledges/ge_StructureGraphVueShare";
import { ge_StructureGraph_GetRecCountByCondCache,ge_StructureGraph_GetSubObjLstCache,ge_StructureGraph_DelRecordAsync,ge_StructureGraph_ReFreshCache,ge_StructureGraph_GetObjByStructureGraphIdAsync,ge_StructureGraph_FuncMapByFldName,ge_StructureGraph_GetObjExLstByPagerCache,ge_StructureGraph_GetObjLstByStructureGraphIdLstAsync,ge_StructureGraph_GetMaxStrIdAsync,ge_StructureGraph_AddNewRecordAsync,ge_StructureGraph_Delge_StructureGraphsAsync } from "@/ts/L3ForWApi/Knowledges/clsge_StructureGraphWApi";
...
if (viewVarSet.sortge_StructureGraphBy == '') viewVarSet.sortge_StructureGraphBy = `${ clsge_StructureGraphEN.con_StructureGraphName } Asc`;
 ...
 **/
public CombineData(arrge_StructureGraphObjLst: Array<clsge_StructureGraphEN>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
...
const strFileName = Format("结构图谱表({0})导出.xlsx",
 clsge_StructureGraphEN._CurrTabName);
const strSheetName = "结构图谱表列表";
 ...
 **/
public async ExportExcel_ge_StructureGraphCache(): Promise<ExportExcelData> 
{
const strThisFuncName = this.ExportExcel_ge_StructureGraphCache.name;
if (viewVarSet.sortge_StructureGraphBy == null)
{
const strMsg = Format("在显示列表时,排序字段(sortge_StructureGraphBy)为空,请检查!(In BindGv_ge_StructureGraphCache)");
...
objge_StructureGraphCond.SetCondFldValue(clsge_StructureGraphEN.con_CourseId, CourseId_Session.value, "=");
 let arrge_StructureGraphObjLst: Array <clsge_StructureGraphEN> = [];
try
{

this.recCount = await ge_StructureGraph_GetRecCountByCondCache(objge_StructureGraphCond ,CourseId_Session.value);
if (this.recCount == 0)
{
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objge_StructureGraphCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
...
arrge_StructureGraphObjLst = await ge_StructureGraph_GetSubObjLstCache(objge_StructureGraphCond ,CourseId_Session.value);
}
catch(e)
{
...
arrge_StructureGraphObjLst = arrge_StructureGraphObjLst.sort(this.SortFunExportExcel);
 return this.CombineData(arrge_StructureGraphObjLst, arrDataColumn); 
 ...
 **/
public async BindTab_ge_StructureGraph(divContainer: HTMLDivElement, arrge_StructureGraphExObjLst: Array<clsge_StructureGraphENEx>) 
{
...
await BindTab(divDataLst, arrge_StructureGraphExObjLst, arrDataColumn,  clsge_StructureGraphEN.con_StructureGraphId, this); 
}
 ...
 **/
public async BindTab_ge_StructureGraph4Func(divContainer: HTMLDivElement, arrge_StructureGraphExObjLst: Array<clsge_StructureGraphENEx>) 
{
 ...
 **/
public async ExtendFldFuncMap(arrge_StructureGraphExObjLst: Array<clsge_StructureGraphENEx>, arrDataColumn: Array<clsDataColumn>) {
...
await ge_StructureGraph_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
lblMsg.innerHTML = Format("根据条件:[{0}]获取的对象列表数为0!", objge_StructureGraphCond.whereCond);
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objge_StructureGraphCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
...
conditionCollection: objge_StructureGraphCond,
orderBy: viewVarSet.sortge_StructureGraphBy,
sortFun: strSortFun
}
arrge_StructureGraphExObjLst = await ge_StructureGraph_GetObjExLstByPagerCache(objPagerPara ,CourseId_Session.value);
}
catch(e)
{
const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, viewVarSet.ascOrDesc4SortFun, viewVarSet.sortge_StructureGraphBy, strSortExpress);
// 将 sortBy 按空格分成两部分
const [sortColumnKey, sortDirection] = sortBy.split(' ');
if (Object.prototype.hasOwnProperty.call(clsge_StructureGraphENEx, sortColumnKey))
{
// 调用 SortColumn 函数
...
try
{
const arrge_StructureGraphObjLst = await ge_StructureGraph_GetObjLstByStructureGraphIdLstAsync(arrStructureGraphId);
 ...
 **/
public Showge_StructureGraphObj(divContainer: HTMLDivElement, objge_StructureGraph: clsge_StructureGraphEN) 
{
if (divContainer == null)
{
