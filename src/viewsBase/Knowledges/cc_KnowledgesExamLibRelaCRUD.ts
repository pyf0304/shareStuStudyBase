/**
 * 类名:cc_KnowledgesExamLibRelaCRUD(界面:cc_KnowledgesExamLibRelaCRUD,01120185)
 * 表名:cc_KnowledgesExamLibRela(01120140)
 * 版本:2025.02.27.1(服务器:WIN-SRV103-116)
 * 日期:2025/03/09 23:52:12
 * 生成者:
 工程名称:问卷调查(0112)
...
import {
  CourseId_Session,
  Combinecc_KnowledgesExamLibRelaConditionObj4ExportExcel,
  Combinecc_KnowledgesExamLibRelaConditionObj,
  divVarSet,
  viewVarSet,
  dataColumn,
  BindTabByList,
...
import {
  ...
  cc_KnowledgesExamLibRela_GetSubObjLstCache,
  cc_KnowledgesExamLibRela_DelRecordAsync,
  cc_KnowledgesExamLibRela_ReFreshCache,
  cc_KnowledgesExamLibRela_GetObjBymIdAsync,
  cc_KnowledgesExamLibRela_GetObjLstBymIdLstAsync,
  cc_KnowledgesExamLibRela_AddNewRecordAsync,
  cc_KnowledgesExamLibRela_UpdateRecordAsync,
  cc_KnowledgesExamLibRela_Delcc_KnowledgesExamLibRelasAsync,
...
import {
  cc_KnowledgesExamLibRelaEx_FuncMapByFldName,
  cc_KnowledgesExamLibRelaEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiExShare/Knowledges/clscc_KnowledgesExamLibRelaExWApi';
...
/** cc_KnowledgesExamLibRelaCRUD 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class cc_KnowledgesExamLibRelaCRUD implements clsOperateList {
  ...
  public static objPageCRUD: cc_KnowledgesExamLibRelaCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    cc_KnowledgesExamLibRelaCRUD.objPageCRUD = this;
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortcc_KnowledgesExamLibRelaBy == '')
        viewVarSet.sortcc_KnowledgesExamLibRelaBy = `${clscc_KnowledgesExamLibRelaEN.con_QuestionId} Asc`;
   ...
   **/
  public CombineData(
    arrcc_KnowledgesExamLibRelaObjLst: Array<clscc_KnowledgesExamLibRelaEN>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
    const intRowNum = arrcc_KnowledgesExamLibRelaObjLst.length;
    ...
    for (let i = 0; i < intRowNum; i++) {
      ...
      const objEN: clscc_KnowledgesExamLibRelaEN = arrcc_KnowledgesExamLibRelaObjLst[i];
      for (let j = 0; j < intColNum; j++) {
   ...
   **/
  public async ExportExcel_cc_KnowledgesExamLibRelaCache() {
    const strThisFuncName = this.ExportExcel_cc_KnowledgesExamLibRelaCache.name;
    ...
    const objcc_KnowledgesExamLibRelaCond =
      await Combinecc_KnowledgesExamLibRelaConditionObj4ExportExcel();
    ...
    try {
      this.recCount = await cc_KnowledgesExamLibRela_GetRecCountByCondCache(
        objcc_KnowledgesExamLibRelaCond,
        CourseId_Session.value,
      );
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objcc_KnowledgesExamLibRelaCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrcc_KnowledgesExamLibRelaObjLst = await cc_KnowledgesExamLibRela_GetSubObjLstCache(
        objcc_KnowledgesExamLibRelaCond,
        CourseId_Session.value,
      );
    } catch (e) {
    ...
    try {
      ...
      arrcc_KnowledgesExamLibRelaObjLst = arrcc_KnowledgesExamLibRelaObjLst.sort(
        this.SortFunExportExcel,
      );
      this.CombineData(arrcc_KnowledgesExamLibRelaObjLst, arrDataColumn);
      //console.log("完成BindGv_cc_KnowledgesExamLibRela!");
    } catch (e) {
  ...
  public async DelRecord(lngmId: number) {
    ...
    try {
      const returnInt = await cc_KnowledgesExamLibRela_DelRecordAsync(lngmId);
      if (returnInt > 0) {
  ...
  public async SelectRecord(lngmId: number) {
    ...
    try {
      ...
      console.log('完成SelectRecord!', objcc_KnowledgesExamLibRelaEN);
      Redirect('/Index/Main_cc_KnowledgesExamLibRela');
    } catch (e) {
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_cc_KnowledgesExamLibRelaCache();
  }

  /** 显示cc_KnowledgesExamLibRela对象的所有属性值
   * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:Gen_Vue_Ts_BindTab)
   * @param divContainer:显示容器，其中包括divDataLst,divPager, divDataLst显示数据, divPager显示分页
   * @param arrcc_KnowledgesExamLibRelaObjLst:需要绑定的对象列表
   **/
  public async BindTab_cc_KnowledgesExamLibRela(
    ...
    if (refcc_KnowledgesExamLibRela_List.value != null) {
      ...
      await BindTabByList(arrcc_KnowledgesExamLibRelaExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_cc_KnowledgesExamLibRela函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async BindTab_cc_KnowledgesExamLibRela4Func(
    ...
    if (refcc_KnowledgesExamLibRela_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_cc_KnowledgesExamLibRela4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
   ...
   * @param arrcc_KnowledgesExamLibRelaExObjLst:需要映射的对象列表
  ...
  public async ExtendFldFuncMap(
    ...
    for (const objDataColumn of arrDataColumn) {
      ...
      for (const objInFor of arrcc_KnowledgesExamLibRelaExObjLst) {
        await cc_KnowledgesExamLibRelaEx_FuncMapByFldName(objDataColumn.fldName, objInFor);
      }
    }
  }
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_cc_KnowledgesExamLibRela4Func(this.listPara.listDiv);
  }
   ...
   **/
  public async BindGv_cc_KnowledgesExamLibRela4Func(divList: HTMLDivElement) {
    ...
    const objcc_KnowledgesExamLibRelaCond = await Combinecc_KnowledgesExamLibRelaConditionObj();
    objcc_KnowledgesExamLibRelaCond.SetCondFldValue(
      clscc_KnowledgesExamLibRelaEN.con_CourseId,
      CourseId_Session.value,
      '=',
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrcc_KnowledgesExamLibRelaExObjLst, true);
        return;
      }
      ...
      arrcc_KnowledgesExamLibRelaExObjLst =
        await cc_KnowledgesExamLibRelaEx_GetObjExLstByPagerCache(
          objPagerPara,
          CourseId_Session.value,
        );
    } catch (e) {
  ...
  public SortFunExportExcel(
    a: clscc_KnowledgesExamLibRelaEN,
    b: clscc_KnowledgesExamLibRelaEN,
  ): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortcc_KnowledgesExamLibRelaBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (clscc_KnowledgesExamLibRelaENEx.hasProperty(sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortcc_KnowledgesExamLibRelaBy = sortBy;
    viewVarSet.ascOrDesc4SortFun = ascOrDesc4SortFun;
    cc_KnowledgesExamLibRelaCRUD.sortFunStatic = sortFun;
    await this.BindGv_cc_KnowledgesExamLibRela4Func(this.listPara.listDiv);
  }
  ...
  public async CopyRecord(arrmId: Array<string>) {
    ...
    try {
      const arrcc_KnowledgesExamLibRelaObjLst =
        await cc_KnowledgesExamLibRela_GetObjLstBymIdLstAsync(arrmId);
      //console.log('responseText=');
      //console.log(responseText);
      let intCount = 0;
      for (const objInFor of arrcc_KnowledgesExamLibRelaObjLst) {
        const returnBool = await cc_KnowledgesExamLibRela_AddNewRecordAsync(objInFor);
   ...
   **/
  public async SetIsMain(arrmId: Array<string>, bolIsMain: boolean) {
    const strThisFuncName = this.SetIsMain.name;
    if (arrmId.length == 0) {
    ...
    try {
      ...
      let intCount = 0;
      for (const objInFor of arrcc_KnowledgesExamLibRelaObjLst) {
        const objcc_KnowledgesExamLibRelaEN = new clscc_KnowledgesExamLibRelaEN();
        ObjectAssign(objcc_KnowledgesExamLibRelaEN, objInFor);
        objcc_KnowledgesExamLibRelaEN.SetmId(objInFor.mId);
        objcc_KnowledgesExamLibRelaEN.SetIsMain(bolIsMain);
        let returnBool = false;
        try {
          objcc_KnowledgesExamLibRelaEN.sfUpdFldSetStr = objcc_KnowledgesExamLibRelaEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await cc_KnowledgesExamLibRela_UpdateRecordAsync(
  ...
  public async DelMultiRecord(arrmId: Array<string>) {
    ...
    try {
      const returnInt = await cc_KnowledgesExamLibRela_Delcc_KnowledgesExamLibRelasAsync(arrmId);
      if (returnInt > 0) {
        cc_KnowledgesExamLibRela_ReFreshCache(CourseId_Session.value);
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   * @param divContainer:显示容器
   * @param objcc_KnowledgesExamLibRela:需要显示的对象
   **/
  public Showcc_KnowledgesExamLibRelaObj(
    divContainer: HTMLDivElement,
    objcc_KnowledgesExamLibRela: clscc_KnowledgesExamLibRelaEN,
  ) {
    if (divContainer == null) {
      alert(Format('所给div为空，divContainer为null!', divContainer));
      return;
    }
    const sstrKeys = GetObjKeys(objcc_KnowledgesExamLibRela);
    const ul: HTMLUListElement = document.createElement('ul');
    for (const strKey of sstrKeys) {
      const strValue = objcc_KnowledgesExamLibRela.GetFldValue(strKey);
   ...
   * @param pobjcc_KnowledgesExamLibRelaEN:表实体类对象
   * @returns 列表的第一个关键字值
   **/
