 ...
 /**
 * 类名:KnowledgeType_Edit(界面:KnowledgeTypeCRUD,01120214)
 * 表名:KnowledgeType(01120890)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/11 00:30:49
 * 生成者:
 工程名称:问卷调查(0112)
...
import { KnowledgeType_GetMaxStrIdAsync,KnowledgeType_CheckPropertyNew,KnowledgeType_AddNewRecordWithMaxIdAsync,KnowledgeType_ReFreshCache,KnowledgeType_GetUniCondStr,KnowledgeType_IsExistRecordAsync,KnowledgeType_GetUniCondStr4Update,KnowledgeType_IsExistAsync,KnowledgeType_GetObjByKnowledgeTypeIdAsync,KnowledgeType_CheckProperty4Update,KnowledgeType_UpdateRecordAsync,KnowledgeType_EditRecordExAsync } from "@/ts/L3ForWApi/Knowledges/clsKnowledgeTypeWApi";
import { CourseId_Session,KnowledgeType_DeleteKeyIdCache,divVarSet,refKnowledgeType_Edit } from "@/viewsShare/Knowledges/KnowledgeTypeVueShare";
...
{
 refKnowledgeType_Edit.value.hideDialog();
}
}
...
if (KnowledgeType_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
if (refKnowledgeType_Edit.value == null)
{
const strMsg = Format(
...
await refKnowledgeType_Edit.value.showDialog(this);
}
...
if (['02', '03', '06'].indexOf(clsKnowledgeTypeEN.PrimaryTypeId) > -1)
{
 ...
 **/
public async btnUpdateRecordInTab_Click(strKnowledgeTypeId: string) {
...
const bolIsSuccess = await this.ShowDialog_KnowledgeType(this.opType);
if (bolIsSuccess == false) return;
this.UpdateRecord(strKnowledgeTypeId);
}
catch (e)
{
 ...
 **/
public async btnUpdateRecord_Click(strKnowledgeTypeId: string) {
...
const update = await this.UpdateRecord(strKnowledgeTypeId);
if (update == false)
{
...
if (this.iShowList != null) this.iShowList.BindGvCache(clsKnowledgeTypeEN._CurrTabName, returnKeyId);
}
}
else
{
...
if (this.iShowList != null) this.iShowList.BindGvCache(clsKnowledgeTypeEN._CurrTabName, this.keyId);
}
}
break;
case "确认修改":
...
refKnowledgeType_Edit.value.Clear();
...
let objKnowledgeTypeEN;
try
{
objKnowledgeTypeEN = await refKnowledgeType_Edit.value.GetEditDataKnowledgeTypeObj();
}
catch(e)
{
...
const strInfo = `添加[知识点类型(KnowledgeType)]记录成功!`;
//显示信息框
...
console.error(strMsg);
        ...
        try {
const objKnowledgeTypeENConst = await KnowledgeType_GetObjByKnowledgeTypeIdAsync(strKnowledgeTypeId);
...
console.error(strMsg);
        ...
        //3、用提供的关键字初始化一个类对象；
refKnowledgeType_Edit.value.ShowDataFromKnowledgeTypeObj(objKnowledgeTypeEN);
}
...
await refKnowledgeType_Edit.value.ShowDataFromKnowledgeTypeObj(objKnowledgeTypeEN);
...
const objKnowledgeTypeEN = await refKnowledgeType_Edit.value.GetEditDataKnowledgeTypeObj();
...
{
KnowledgeType_ReFreshCache(CourseId_Session.value);
KnowledgeType_DeleteKeyIdCache(CourseId_Session.value, this.keyId);
}
return returnBool;
}
catch(e)
...
const returnBool = await KnowledgeType_EditRecordExAsync(objKnowledgeTypeEN);
if (returnBool == true)
{
KnowledgeType_ReFreshCache(CourseId_Session.value);
}
...
refKnowledgeType_Edit.value.strCancelButtonText = value;
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmitKnowledgeType():string {
const strValue = refKnowledgeType_Edit.value.strSubmitButtonText;
return strValue;
}
 /**
...
refKnowledgeType_Edit.value.strSubmitButtonText = value;
}
}